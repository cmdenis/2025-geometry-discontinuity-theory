...
# ╔═╡ e874f768-b34f-11ef-14b5-c34847856a6e
using DifferentialEquations, CairoMakie, PlutoUI, FileIO

# ╔═╡ 8f2145e2-d255-4685-ae60-7104f96cff8b
...
# ╔═╡ ea28a182-bef1-41ab-a40c-055dfda126c7
function RELU(x)
	return max(0, x)
end

# ╔═╡ d06b37bc-46d0-4805-9a2f-1d8cf57fdd5b
...
# ╔═╡ c3aebfb4-90c4-4fb5-8905-8669aeebfc11
...
# ╔═╡ 4d6b76b5-f22d-49a4-b1d9-de64576d795b
function fScaled(du, u, p, t)
	du[1], du[2], du[3] = diffEquationScaled(u[1], u[2], u[3], p)
end

# ╔═╡ 5d7c3ae1-0750-4018-8809-c56d8867ac0e
function integrateScaled(p, u0, times, infection_time, pathogenAmount; pad = false)
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = p
	...
	affect!(integrator) = (integrator.u[3] += pathogenAmount)
	cb = DiscreteCallback(condition, affect!, save_positions = (false, false));
	

	# define problem
	prob = ODEProblem(fScaled, [u1, u2, u3], (times[1], times[end]), p, callback = cb)

	# solve system
	sol = solve(prob, Rodas4P(), saveat = times, abstol=1e-9, reltol=1e-6, tstops=[infection_time])
...
# ╔═╡ 906b7363-bc82-4c44-a90b-5c724b6648f8
function isacute(p, u0, infectionTime, pathogenAmount; threshhold = 1e-3)
	# function that outputs if the system is undergoing an acute reaction or not

	times = LinRange(0., 1000, 4000)
	sol = integrateScaled(p, u0, times, infectionTime, pathogenAmount)
...
# ╔═╡ 5aa53d77-932c-443d-beab-e4b2fbc72690
function diffEquation(x, y, p)
...
# ╔═╡ 23bebc2b-83e0-44cc-b802-f3fdbd02c33f
baseU0 = (
	0.0,
	10.0,
	10.0
...
# ╔═╡ fc1b7368-27df-466c-8d24-c76215c0c1a8
function showRuns(p, u0, fig, treatmentTime, pathogenAmount; nameLabel = "", showyaxis1 = true, showyaxis2 = true, showxaxis = true)

	# times for simulation
	times = LinRange(0, 100, 2000)

	# compute solution
	sol = integrateScaled(p, u0, times, treatmentTime, pathogenAmount)
	...
	ax1 = Axis(
		...
		yticklabelsize = 20,
			...
			xlabelsize = 20,
			ylabelsize = 20,
		xticklabelsvisible = showxaxis,
		xlabelvisible = showxaxis,
		yticklabelsvisible = showyaxis1,
		ylabelvisible = showyaxis1,
		yticks = ([0.01, 1, 100], ["10⁻²", "10⁰", "10²"])
	)
	
	ax2 = Axis(fig[1, 1], yticklabelcolor = T_color, ylabel = "T-Cells", yscale = log10, ygridvisible = false, yticklabelsize = 20,
			xticklabelsize = 20,
			xlabelsize = 20,
			ylabelsize = 20, yticklabelsvisible = showyaxis2, ylabelvisible = showyaxis2)
	xlims!(ax1, 0, times[end])
	xlims!(ax2, 0, times[end])
	ylims!(ax1, 0.007, 900)
	lines!(ax2, times, T_traj, color = T_color, linewidth = 4, alpha = 0.9)
	lines!(ax1, times, I_traj, color = I_color, linewidth = 4, alpha = 0.9)
	lines!(ax1, times, I_traj_2, color = I_color_2, linewidth = 4, alpha = 0.9)
...
# ╔═╡ cd5d3a48-403b-4528-be6f-3944d9ac09e3
function solveSystem(p, u0, times)
	#sol = integrate(f, params, [(10^u01)*(p10/p7), (10^u02)*(p7/p1)], times);
	# Parameters
	...
	u1 = (u0[1])*(kappa/gamma)
	u2 = (u0[2])*(gamma/g)
	times_scaled = times*gamma

	# Integrate the scaled version
	prob = ODEProblem(
		fScaled, 
		[u1, u2, 0], 
		(times_scaled[1], times_scaled[end]), 
		[g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 3]
	)
	
	sol = solve(prob, Rodas4P(), saveat = times_scaled, abstol=1e-9, reltol=1e-6)

	sol1 = sol[1, :]*gamma/kappa
	sol2 = sol[2, :]*g/gamma

	return hcat(sol1, sol2)'
end

# ╔═╡ 94670d65-66a7-4cb9-ba3f-de47c4facd17
function isacute(p, u0; threshhold=0.01)
	# Parameters
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = p
	...
	prob = ODEProblem(
		...
		[u1, u2, 0], 
		(tt[1], tt[end]), 
		[g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 1]
	)
...
# ╔═╡ 13546294-a1f5-4fb1-b7dd-0e56a8223217
function makePhaseDiagram1(fig, xpoints, ypoints, labels)
	g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = baseP
	u1, _, u3 = baseU0
	u2 = 0.

	xx, yy, mat = outcomeMatrix([g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2], [u1, 10, 1])
	ax1, hm = heatmap(
		fig[2, 1], 
		xx, 
		yy, 
		...
		axis = (;xlabel = "Growth rate (r₂)",
			xticks = ([0.1, 1, 10], ["10⁻¹", "10⁰", "10¹"]),
			#ylabel = L"I_2(0)", 
			ylabel = "Initial Challenge",
			xscale = log10, 
			yscale = log10,
	...
	xx, yy, mat = outcomeMatrix([g, 3.0, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2], [u1, u2, 0])
	contour!(ax1, xx, yy, mat, levels = [1], color = :red, linewidth = 4, linestyle = :dot)

	scatter!(ax1, xpoints, ypoints, color = :white, strokewidth = 1, strokecolor = :black, markersize = 15)
	text!(ax1, xpoints, ypoints, text = labels, offset = (10, -15), fontsize = 30, color = :white, strokecolor = :black, strokewidth = 1, font = :bold)
	...
	cb = Colorbar(
		fig[1,1], 
		hm, 
		ticks = ([0, 1, 2, 3], ["I₁ Chronic\nI₂ Chronic", "I₁ Acute\nI₂ Chronic", "I₁ Chronic\nI₂ Acute", "I₁ Acute\nI₂ Acute"]), labelsize = 20, vertical = false, 
			ticklabelsize = 20, ticklabelrotation = 0)

	return ax1
end

# ╔═╡ a975ea24-e509-450a-a75c-9d967c14982d
...
# ╔═╡ 0cd3d31e-72fe-43fd-9f2f-1ea822e8cdf1
function makeStreamPlot(fig, params, (minX, maxX, minY, maxY); show_bassin=true)
	...
	times = 0:0.01:10
	sol = solveSystem(params, [0, 1], times);
...
# ╔═╡ d16edc18-b363-40bc-998d-c39f8dd46161
let
	...
	#img = load(assetpath(string(pwd(), "/inkscape/fig_treatment_concept.png")))

	ax_A = Axis(fig[1, 1:3], aspect = DataAspect(), 
			yreversed = true,)
	#image!(
	#	ax_A, 
	#	img',
	...
	examplesFig = fig[2, 2] = GridLayout()

	
	figs = [examplesFig[1, 1], examplesFig[1, 2], examplesFig[2, 1], examplesFig[2, 2]]


	makePhaseDiagram1(fig[2, 1:2], r2s, i2s, labels)

	#showRuns([u1, u2, 0.], baseP, fig[2, 2], 50, 140; nameLabel = "")
	
	showRuns(
			[g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 1],
			[0, 10., 0.], 
			fig[2, 3], 50, 300; nameLabel = "", showxaxis=true, showyaxis2=true)
	...
	showRuns(
			[g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 1],
			[0, 10., 0.], 
			underSub[1, 1], 50, 0.2; nameLabel = "", showxaxis=true, showyaxis2=false)
	showRuns(
			[g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 6],
			[0, 10., 0.], 
			underSub[1, 2], 50, 10; nameLabel = "", showyaxis2=false, showyaxis1=false)
	showRuns(
			[g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 1],
			[0, 10., 0.], 
			underSub[1, 3], 50, 10; nameLabel = "", showyaxis1=false)

	
	#rowgap!(examplesFig, -0.0)
	#colgap!(examplesFig, 10.0)
	...
	# Labels
	label_size = 35
	text!(fig.scene, 0, 1, text = "A)", fontsize = label_size, font = :bold, space = :relative, offset = (0, 0), align = (:left, :top))

	relHeight2 = 0.53
	relHeight3 = 0.29
	relV2 = 0.419
	relV3 = 0.7
	colorPalette = cgrad(:Paired_10, 4; categorical = true)
	text!(fig.scene, 0, 0.65, text = "B)", fontsize = label_size, font = :bold, space = :relative, offset = (0, -0), align = (:left, :top))
	text!(fig.scene, 0.655, 0.625, text = "C)", fontsize = label_size, font = :bold, space = :relative, offset = (0, -0), align = (:left, :top), color = colorPalette[4], strokecolor = :black, strokewidth = 1)
	text!(fig.scene, 0.11, 0.31, text = "D)", fontsize = label_size, font = :bold, space = :relative, offset = (0, -0), align = (:left, :top), color = colorPalette[3], strokecolor = :black, strokewidth = 1)
	text!(fig.scene, 0.38, 0.31, text = "E)", fontsize = label_size, font = :bold, space = :relative, offset = (0, -0), align = (:left, :top), color = colorPalette[2], strokecolor = :black, strokewidth = 1)
	text!(fig.scene, 0.655, 0.31, text = "F)", fontsize = label_size, font = :bold, space = :relative, offset = (0, -0), align = (:left, :top), color = colorPalette[1], strokecolor = :black, strokewidth = 1)
	

	save("makieExports/fig6.png", fig)
...
# ╔═╡ 422f7b6f-2e1b-4a6a-8fc2-d87b46326bec
let
	...
	examplesFig = fig[1, 1] = GridLayout()
	...
	#showRuns([u1, u2, 0.], baseP, fig[2, 2], 50, 140; nameLabel = "")
	for (i2, r2, label, ff) in zip(i2s, r2s, labels, figs)
		showRuns(
			[g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2],
			[0, 10., 0.], 
			ff, timing, i2; nameLabel = "")
	end
...
manifest_format = "2.0"
project_hash = "4d87c6fedb0fd051bdab8003d16207b91322be6c"
