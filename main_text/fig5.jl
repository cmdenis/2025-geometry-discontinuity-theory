...
# ╔═╡ 2f3caee8-acbe-11ef-03ae-275995444287
using DifferentialEquations, CairoMakie, Roots, ForwardDiff, LinearAlgebra, PlutoUI, FileIO

# ╔═╡ 87df5d38-e1c2-4b92-a2aa-1c68a8b00751
import Plots.palette

# ╔═╡ ecb466ac-468a-4ea3-bb04-43a3fa2f4877
function convertParam(p)
    # function to convert the original parameters to the c1, c2, c3 parameters
    g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = p

    # creating the new parameters
    c1_1 = (r1 - kappa * T_star) / gamma
	c1_2 = (r2 - kappa * T_star) / gamma
    c2 = (g * T_star * kappa^2) / (gamma^3)
    c3 = (alpha * delta2 * g * kappa) / (beta * delta1 * gamma^2)
    return c1_1, c1_2, c2, c3
end

# ╔═╡ 62e47543-e58e-4e6c-971a-8e1a0d84b651
function RELU(x)
	return max(0, x)
end

# ╔═╡ 435d80fb-60d7-4b15-b05d-613320055f27
getRange(x) = maximum(x) - minimum(x) # helper function for the range of values for a series

# ╔═╡ 54ae75c9-3157-408b-97a4-6fca0dbd4fb5
function crossfader(x, n, k)
	# function to crossfade between the below one threshold and the existence of the virus
	return (x/k)^n/((x/k)^n + 1)
end

# ╔═╡ 0d82eacf-6c98-456d-b0d5-ca2b43198426
function diffEquationScaled(xp, yp, zp, p)
	# outputs the derivatives of the system of equation on a natural log scale
	
	# Parameters
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = p
    c1_1, c1_2, c2, c3 = convertParam(p)
	...
	# T-Cells (log scale)
    dx = c2 * (y + z) + x * (c3 * (y + z) - 1)
	#dxp = dx / x


	# infected cells 1 (log scale)
    n = 4
    k = gamma/g
    dy = (1 + y * (c1_1 - x)) * crossfader(y, n, k) - (1 - crossfader(y, n, k)) * y
	#dyp = dy/y

	# infected cells 2 (log scale)
    n = 4
    k = gamma/g
    dz = (1 + z * (c1_2 - x)) * crossfader(z, n, k) - (1 - crossfader(z, n, k)) * z
	#dzp = dz/z
...
# ╔═╡ f1ca1b47-273b-4728-ada1-a25370523c11
function fScaled(du, u, p, t)
	du[1], du[2], du[3] = diffEquationScaled(u[1], u[2], u[3], p)
end

# ╔═╡ 9cf124e6-6356-4c24-8cc2-fe33e77598bb
function diffEquation1Species(x, y, p)
	
	# Parameters
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = p
    c1, c1_2, c2, c3 = convertParam(p)

	# rescaling variables
	xp = RELU(x)#/(gamma/kappa)
	yp = RELU(y)#/(g/gamma)

	# differential equations
    du1 = c2 * (yp) + xp * (c3 * (yp) - 1)# T-cells

    n = 4
    k = gamma/g

    du2 = (1 + yp * (c1 - xp)) * crossfader(yp, n, k) - (1 - crossfader(yp, n, k)) * yp # infected cells 
    return du1, du2
end

# ╔═╡ 15948f9c-3bba-4ed2-a706-95291f60132d
function f1Species(du, u, p, t)
    du[1], du[2] = diffEquation1Species(u[1], u[2], p)
end

# ╔═╡ 2db5a473-45ba-434b-b823-d0c6e702a198
function logSpaceDiffEquation(x, y, p)
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = p
	
	scaled_x = (10^x)*(kappa/gamma)
	scaled_y = (10^y)*(gamma/g)
	
    dx, dy = diffEquation1Species(scaled_x, scaled_y, p)
	
    return Point2f([dx / (scaled_x), dy / scaled_y])
end

# ╔═╡ 8f0be684-5a40-4985-975a-8f36d0fab2f6
function integrateScaled(u0, p, times; pad = false)
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = p
	# function to integrate system
	u1 = u0[1]*(kappa/gamma)
	u2 = u0[2]*(gamma/g)
	u3 = u0[3]*(gamma/g)

	# define callback function to terminate integration (for blowups)

	# define problem
	prob = ODEProblem(fScaled, [u1, u2, u3], (times[1], times[end]), p)

	# solve system
	sol = solve(prob, Rodas4P(), saveat = times, abstol=1e-9, reltol=1e-6)

	# scale back solution to normal scale
	sol1 = sol[1, :]/(kappa/gamma)
	sol2 = sol[2, :]/(gamma/g)
	sol3 = sol[3, :]/(gamma/g)

	# Pad array with small values 
	if pad != false
		padAmount = length(times) - length(sol1) 
		sol1 = vcat(sol1, fill(pad, padAmount))
		sol2 = vcat(sol2, fill(pad, padAmount))
		sol3 = vcat(sol3, fill(pad, padAmount))
	end
	
	return hcat(sol1, sol2, sol3)'
end

# ╔═╡ 4b30df24-1daf-468f-a473-6e664e77057f
baseP = (
	g = 10.0,
	r1 = 0.2,
	T_star = 100.0,
...
# ╔═╡ 6f55b903-e00c-40f4-bc9b-1d426bcb48b7
let 
	g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = baseP

	convertParam([g, 1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 0])
	
end

# ╔═╡ 4e28b3e4-1c6c-44d7-b04a-23b4ab940072
baseU0 = (
	0.0,
	10,
	10
...
# ╔═╡ f618da2a-a0e2-4e23-877e-5df7e4911132
let 
	fig = Figure()

	ax1 = Axis(fig[1, 1], yscale = log10, ylabel = "T-cells")
	ax2 = Axis(fig[2, 1], yscale = log10, ylabel = "infected cells")

	# Get solution
	times = 0:0.1:50
	sol = integrateScaled(baseU0, baseP, times, pad = NaN)

	lines!(ax1, times, sol[1, :])
	lines!(ax2, times, sol[2, :])
	lines!(ax2, times, sol[3, :])

	fig
end
	

# ╔═╡ ff2d6478-e79a-420f-b75e-ea953f7dccf4
function isacute(p, u0; threshhold = 1e-3)
	# function that outputs if the system is undergoing an acute reaction or not

	times = LinRange(0., 1000, 4000)
	sol = integrateScaled(u0, p, times)

	acuteI1 = (sol[2, end] < 1e-2)
	acuteI2 = (sol[3, end] < 1e-2)
...
# ╔═╡ 5251de67-0b1b-4760-a786-cc046b99fae2
md"## Initial antagonism plots
Here we make the figures for antagonism, but only looking for the antagonism happening for two species being put at the same time (and also only for the reduces model).
"

# ╔═╡ 7c5f386a-ac23-42f6-bd29-fd30b07b8131
function isacute2Species(p, u0; threshhold = 0.01)
	# Parameters
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = p
    ...
    # Integrate the scaled version
	prob = ODEProblem(
		fScaled, 
		[u1, u2, u3], 
		(tt[1], tt[end]), 
		[g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2]
	)
	#sol = solve(prob, AutoTsit5(Rosenbrock23()), saveat = tt)
	#sol = solve(prob, Euler(), dt = 0.001, saveat = tt)
	sol = solve(prob, Rodas4P(), saveat = tt, abstol=1e-9, reltol=1e-6)
	#sol = solve(prob, Rodas4P(), saveat = times)
	
	

    # check if system is acute
    if sol[2, end] < threshhold
        return 1
    else
        return 0
    end
...
# ╔═╡ 233b4eb8-b07a-4a73-b7c7-65dd345b6bfc
function outcomeMatrix2Species(p, u0)

	g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = p
	_, u2, u3 = u0
	I1s = 10 .^ LinRange(-1, 3., 101)
	u1s = 10 .^ LinRange(-3, 6., 101)

	mat = fill(NaN, size(u1s)..., size(I1s)...)

	for (i, I1) in enumerate(I1s)
		for (j, u1) in enumerate(u1s)
			mat[j, i] = isacute2Species(
				[g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2],
				[u1, I1, u3],
...
# ╔═╡ 2f25ce9f-b2b6-445e-80e1-3f5ec78e16b5
function isacute(p, u0, system; threshhold=0.01)
	# Parameters
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = p
...
# ╔═╡ a57c9749-f6ba-48eb-b4e1-af24a0b9cd1f
isacute(baseP, baseU0)

# ╔═╡ eba2a268-be2e-4895-9a8b-64ad17353cfe
function outcomeMatrix(p, u0)

	g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, _ = p
	u1, u2, u3 = u0
	I2s = 10 .^ LinRange(-1, 3., 100)
	r2s = 10 .^ LinRange(-1, 1.1, 100)

	mat = fill(NaN, size(r2s)..., size(I2s)...)
...
# ╔═╡ f0745926-bdf8-4aea-a44f-f1df606558dd
function outcomeMatrixR1I2(p, u0)

	g, _, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = p
	u1, u2, u3 = u0
	I2s = 10 .^ LinRange(-1, 3., 100)
	r1s = 10 .^ LinRange(-1, 1.1, 100)

	mat = fill(NaN, size(r1s)..., size(I2s)...)
...
# ╔═╡ 82a5653d-2947-4279-9299-d8af962d2f00
function makeBassins(xlimits, ylimits, p)
    # function to create the bassin of attractions 
    ...
    xSize = 21
    ySize = 21
    mat = fill(NaN, xSize, ySize)
    xspace = range(xlimits...; length=xSize)
    yspace = range(ylimits...; length=ySize)

    for (i, xx) in enumerate(xspace)
        #println("x:", xx)
        for (j, yy) in enumerate(yspace)
			if isacute(p, [10^xx, 10^yy], f1Species) == false
                mat[i, j] = 1
            end
			
        end
    end
    return xspace, yspace, mat
end

# ╔═╡ e6d84978-dce7-4da3-b45f-54e357b54a07
function plotRuns(p, u0, fig; showLegend = false, showI1 = true, showI2 = true, title = "")

	# times for simulation
	times = LinRange(0, 80, 2000)


	# compute solution
	sol = integrateScaled(u0, p, times)

	# removing values that are too low from trajectories (for plotting scale)
	T_traj = sol[1, :]
	T_traj[T_traj .< 1e-3] .= NaN
	
	I1_traj = sol[2, :]
	I1_traj[I1_traj .< 1e-3] .= NaN
	I2_traj = sol[3, :]
	I2_traj[I2_traj .< 1e-3] .= NaN
	...
	ax1 = Axis(
		fig[1, 1], 
		yticklabelcolor = I1_color, 
		ylabel = "Immune\nChallenge", 
		xlabel = "Time",
		yscale = log10,
		ylabelcolor = I1_color,
		#title = title,
		...
		yticks = (10.0 .^(-2:2:4), ["10⁻²", "10⁰", "10²", "10⁴"]),
		xlabelpadding = -5
	)
	
	ax2 = Axis(fig[1, 1], yticklabelcolor = T_color, ylabel = "T-Cells", yscale = log10, xgridvisible = false, ygridvisible = false, yticklabelsize = 20,
			xticklabelsize = 20,
			ylabelsize = 20,
			xlabelsize = 20,
			yticks = (10.0 .^(-2:2:4), ["10⁻²", "10⁰", "10²", "10⁴"]),
	...
	xlims!(ax1, 0, times[end])
	ylims!(ax1, 0.1, 10^2.8)
	xlims!(ax2, 0, times[end])

	lines!(ax2, times, T_traj, color = T_color, linewidth = 4, alpha = alphaAmount)
	if showI1
		lines!(ax1, times, I1_traj, color = I1_color, linewidth = 4, alpha = alphaAmount)
	end
	if showI2
		lines!(ax1, times, I2_traj, color = I2_color, linewidth = 4, alpha = alphaAmount)
	end

	# adjust the axis so that it fits on the otherside of the plot
	...
	if showLegend
		Legend(fig[1, 1],
	    	[
				[LineElement(color = T_color, linewidth = 4, alpha = alphaAmount)],
				[LineElement(color = I1_color, linewidth = 4, alpha = alphaAmount)],
				[LineElement(color = I2_color, linewidth = 4, alpha = alphaAmount)]
			],
	    	[L"T", L"I_1", L"I_2"], tellwidth = false, tellheight = false, orientation = :horizontal, halign = :right, valign = :bottom, labelsize = 20
		)
	end
...
# ╔═╡ 442e2ee7-0a8e-4824-af00-4250625d1ffb
function plotTrajectories(p, u0, fig; showLegend = false, showI1 = true, showI2 = true, title = "")
	...
	# removing values that are too low from trajectories (for plotting scale)
	T_traj = sol[1, :]
	T_traj[T_traj .< 1e-4] .= NaN
	
	I1_traj = sol[2, :]
	I1_traj[I1_traj .< 1e-2] .= NaN
	I2_traj = sol[3, :]
	I2_traj[I2_traj .< 1e-2] .= NaN
	...
	ax1 = Axis(
		...
		yticklabelsize = 20,
			...
			ylabelsize = 20,
			xlabelsize = 20,
		yticks = (-1:3, ["10⁻¹", "10⁰", "10¹", "10²", "10³"]),
		xticks = (-3:3:6, ["10⁻³", "10⁰", "10³", "10⁶"])
	)
	...
	if showI1 
		y = log10.(abs.(I1_traj))
		c = LinRange(1, times[end], length(x))
		#obj = lines!(ax1, x, y; color=c, colormap=(:hot), linewidth=4)
		lines!(ax1, x, y; color=:orange,  linewidth=4)
    	#Colorbar(fig[1, 2], obj, ticksvisible = false, ticklabelsvisible = false)
	end
	if showI2 
		y = log10.(abs.(I2_traj))
		c = LinRange(1, times[end], length(x))
		#obj = lines!(ax1, x, y; color=c, colormap=(:autumn1), linewidth=4)
		lines!(ax1, x, y; color=:tomato1, linewidth = 4)
    	#Colorbar(fig[1, 3], obj, label="Time (days)")
	end
	...
	if showLegend
		Legend(fig[1, 1],
	    	...
	    	[L"T", L"I_1", L"I_2"], tellwidth = false, tellheight = false, orientation = :horizontal, halign = :right, valign = :bottom
		)
	end

	ax1.alignmode = Mixed(right = -70)

	return ax1

end

# ╔═╡ 85bcb5b8-9afc-4559-a035-a556e42e00e5
function makeStreamPlot(fig, params, u0, lims; show_bassin=true, showI1 = false, showI2 = false, colorMap = :hot, color = :orange)
    # function to make streamplot on fig
	
	# Parameters
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = params
    ...
    rightPannelAx1 = Axis(
		...
		xlabel="T-Cells", 
		ylabel="Immune\nChallenge",
		#title = string("g=", g, "\tr=", r, "\tT*=", T_star, "\nT*reg=", T_star_reg, "\tα=", alpha, "\tβ=", beta, "\nγ=", gamma, "\tδ1=", delta1, "\tδ2=", delta2, "\nκ=", kappa, "\tμ=", mu)
		xgridvisible = false, ygridvisible = false,
		yticklabelsize = 20,
			xticklabelsize = 20,
			ylabelsize = 20,
    ...
    FX = [logSpaceDiffEquation(x, y, params)[1] for x in xs, y in ys]
    FY = [logSpaceDiffEquation(x, y, params)[2] for x in xs, y in ys]
    contour!(rightPannelAx1, xs, ys, FX; levels=[0.0], color=:black, linestyle=:dash)
    contour!(rightPannelAx1, xs, ys, FY; levels=[0.0], color=:black, linestyle=:dash)


    if show_bassin == true
        # compute bassin of attraction
        xx, yy, mat = makeBassins((minX, maxX), (minY, maxY), params)
        heatmap!(rightPannelAx1, xx, yy, mat, colormap=:bone, alpha=0.2)
    end

    streamplot!(
        rightPannelAx1,
        (x, y) -> logSpaceDiffEquation(x, y, params),
        lims[1:2],
        lims[3:4],
        density=0.4,
        colormap=[:grey70],
	...
	times = 0:0.1:80
	sol = integrateScaled(u0, params, times)
	...
	if showI1
    	y = log10.(abs.(sol[2, :]))
		c = LinRange(1, times[end], length(x))
		#obj = lines!(rightPannelAx1, x, y; color=c, colormap=colorMap, linewidth=4)
		lines!(rightPannelAx1, x, y, color=color, alpha = alphaAmount, linewidth = 4)
    #Colorbar(fig[1, 2], obj, label="Time (days)")
	end
	if showI2
		y = log10.(abs.(sol[3, :]))
		c = LinRange(1, times[end], length(x))
	...
	return rightPannelAx1

end
		


# ╔═╡ a78799aa-ee53-4ea8-bcc7-93dbb925633f
function makePhaseDiagram1(fig, low_r1, r2_cross_section)
	g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = baseP
	u1, u2, u3 = baseU0

	xx, yy, mat = outcomeMatrix([g, low_r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2], [u1, 100, 1])
	ax1, hm = heatmap(
		fig[1, 1], 
		xx, 
		yy, 
		mat,
		colormap = cgrad(:Paired_10, 4; categorical = true),
		colorrange = (-0.5, 3.5),
		nan_color = :white,
		axis = (;xlabel = "Growth Rate (r₂)",#L"r_2", 
			ylabel = "Initial Challenge (I₂)", 
			xscale = log10, 
			yscale = log10,
	...
	vlines!(ax1, [r2_cross_section], linestyle = :dot, color = :black, linewidth = 4)
	xx, yy, mat = outcomeMatrix([g, 3.0, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2], [u1, 0.0001, 10])
	contour!(ax1, xx, yy, mat, levels = [1], color = :red, linewidth = 4, linestyle = :dot)

	#scatter!(ax1, [r2_cross_section], [20], color = :white, strokecolor = :black, strokewidth = 1, markersize = 12)
	#text!(ax1, [r2_cross_section], [20], text = "4E)", offset = (4, 4), color = :white, fontsize = 30, font = :bold)


	#cb = Colorbar(
	#	fig[1,2], 
	#	hm, 
	#	ticks = ([0, 1, 2, 3], ["I₁ Chronic\nI₂ Chronic", "I₁ Acute\nI₂ Chronic", "#I₁ Chronic\nI₂ Acute", "I₁ Acute\nI₂ Acute"]),
		#ticklabelsize = 20,
		#labelsize = 20,
		#label = "Response")
...
# ╔═╡ da13a7ce-0ad8-48a1-9dbb-c351b0bb3c40
function makePhaseDiagram2(fig, low_r1, r2_cross_section)
	g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = baseP
	u1, u2, u3 = baseU0

	xx, yy, mat = outcomeMatrixR1I2([g, low_r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2_cross_section], [u1, 100, 1])
	ax2, hm = heatmap(
		fig[1, 1], 
		xx, 
		yy, 
	...
	#scatter!(ax2, [low_r1], [20], color = :white, strokecolor = :black, strokewidth = 1, markersize = 12)
	#text!(ax2, [low_r1], [20], text = "4E)", offset = (4, 4), color = :white, fontsize = 30, font = :bold)
...
# ╔═╡ f2246e3e-f2f8-4f25-8def-8a01cebe90b8
let 
	update_theme!(
		Theme(xgridvisible = false, ygridvisible = false, yticklabelsize = 20)
	)
	fig = Figure(size = (900, 1600))
	...
	low_r1 = 0.2
	high_r2 = 2.5
	r2_cross_section = 3.0

	g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = baseP
	u1, u2, u3 = baseU0

	
	ax_A = Axis(fig[1, 1:2], aspect = DataAspect(), 
			yreversed = true,)
	hidedecorations!(ax_A)
	hidespines!(ax_A)
	rowsize!(fig.layout, 1, 280)
	#rowsize!(fig.layout, 1, Aspect(1, 1))



	# Example of combination of runs
	combinationFig = fig[2, 1:2] = GridLayout()
	

	ax1 = plotRuns(
		[g, low_r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, high_r2], 
		[u1, 10, 0.0], 
		combinationFig[1, 1], 
		showI2 = false, title = "I₁ alone (chronic)", showLegend=true)

	#plotTrajectories([g, low_r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, high_r2], 
	#	[u1, 10, 0.0], combinationFig[1, 2])

	ax2 = makeStreamPlot(combinationFig[2, 1], [g, low_r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 0.0], [u1, 10, 0.0], (-1, 6, -1, 3), show_bassin=true, showI1=true, colorMap=:autumn1)
	...
	# showing run of I2 alone
	ax1 = plotRuns(
		[g, low_r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, high_r2], 
		[u1, 0.0, 10], 
		combinationFig[1, 2], 
		showI1 = false, title = "I₂ alone (acute)", showLegend=true)

	#plotTrajectories([g, low_r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, high_r2], 
	#	[u1, 0.0, 10], combinationFig[2, 2])

	ax2 = makeStreamPlot(combinationFig[2, 2], [g, high_r2, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 0.0], [u1, 10, 0.0], (-1, 6, -1, 3), show_bassin=true, showI1=true, colorMap=:hot, color = :tomato1)
	rowgap!(combinationFig, 8)
	...
	# Now presenting combinations
	ax_A = Axis(fig[3, 1:2], aspect = DataAspect(), 
			yreversed = true,)
	hidedecorations!(ax_A)
	hidespines!(ax_A)
	rowsize!(fig.layout, 3, 350)
	
	ax = plotRuns(
		[g, low_r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, high_r2], 
		[u1, 10., 10.], 
		fig[4, 1], title = "I₁ and I₂ (acute and chronic)",
		showLegend=true
	)
	ax.xlabelpadding = 5


	plotTrajectories([g, low_r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, high_r2], [u1, 10., 10.], fig[4, 2])

	

	# Phase diagram 1
	phaseFig  = GridLayout(fig[5, :], right = 0)
	makePhaseDiagram1(phaseFig[1, 1], low_r1, r2_cross_section)
	makePhaseDiagram2(phaseFig[1, 2], low_r1, r2_cross_section)
	colsize!(phaseFig, 1, Relative(0.40))


	


	rowsize!(fig.layout, 4, 130)
	rowsize!(fig.layout, 5, Relative(0.18))
	

	label_size = 30
	xoff = 10
	yoff = -label_size
	text!(fig.scene, 0, 1, text = "A)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	text!(fig.scene, 0.5, 1, text = "B)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	text!(fig.scene, 0, 0.81, text = "C)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	text!(fig.scene, 0.5, 0.81, text = "D)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	text!(fig.scene, 0, 0.54, text = "E)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	text!(fig.scene, 0, 0.35, text = "F)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	text!(fig.scene, 0, 0.2, text = "G)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	#text!(fig.scene, labV1, labH4, text = "H)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	#text!(fig.scene, labV2, labH4, text = "I)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	#text!(fig.scene, labV3, labH2, text = "J)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	#text!(fig.scene, labV1, labH5, text = "K)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	#text!(fig.scene, 0.5, labH5, text = "L)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))

	save("makieExports/fig5.png", fig)
	

	fig
end

# ╔═╡ 40196b6a-a288-450b-9925-a6994b36d970
# ╠═╡ disabled = true
#=╠═╡
let 
	update_theme!(
		Theme(xgridvisible = false, ygridvisible = false)
	)
	...
	img = load(assetpath(string(pwd(), "/fig_multispecies_concept.png")))

	ax_A = Axis(fig[1, 1], aspect = DataAspect(), 
			yreversed = true,)
	image!(
		ax_A, 
		img',
	...
	# Example of combination of runs
	combinationFig = fig[1, 2] = GridLayout()
	...
	plotRuns(
		...
		[u1, 10, 0.0], 
		combinationFig[1, 1], 
		showI2 = false, title = "I₁ alone (chronic)")

	#plotTrajectories([g, low_r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, high_r2], 
	#	[u1, 10, 0.0], combinationFig[1, 2])

	makeStreamPlot(combinationFig[1, 2], [g, low_r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 0.0], [u1, 10, 0.0], (-1, 6, -1, 3), show_bassin=true, showI1=true, colorMap=:autumn1)
	...
	makeStreamPlot(combinationFig[2, 2], [g, high_r2, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 0.0], [u1, 10, 0.0], (-1, 6, -1, 3), show_bassin=true, showI1=true, colorMap=:hot, color = :tomato1)


	plotRuns(
		[g, low_r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, high_r2], 
		[u1, 10., 10.], 
		combinationFig[3, 1], title = "I₁ and I₂ (acute and chronic)")


	plotTrajectories([g, low_r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, high_r2], [u1, 10., 10.], combinationFig[3, 2])

	

	colgap!(combinationFig, 20.0)
	rowgap!(combinationFig, 0.0)
	colsize!(combinationFig, 1, Relative(2/3))
	#colsize!(fig.layout, 1, 500)

	

	contourfig = fig[1, 3] = GridLayout()
	ax6 = Axis(contourfig[1, 1], 
		xlabel = "T-Cells",
		ylabel = "Infected Cells",
		xscale = log10, yscale = log10,
		limits = (1e-3, 1e6, 1e-1, 1e3),
		xgridvisible = false, ygridvisible = false
		#xgridvisible = false, 
		#ygridvisible = false, 
	...
	#colors = [:lightskyblue1, :deepskyblue1, :dodgerblue1, :dodgerblue4]
	#colors = [palette(:PuBu_5)[i] for i in 1:5]
	colors = [:black, :grey40, :grey60, :grey80]
	vals = [0.0, 10, 300, 330]

	for (I2, color) in zip(vals, colors)
		xx, yy, mat = outcomeMatrix2Species(
			[g, 2.5, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2],
			[u1, u2, I2]
		)
	
	
		contour!(ax6, (xx), (yy), mat,
			levels=[0, 1], linewidth = 4, color = color, alpha = 0.3
		)
	end

	# compute solution
	sol = integrateScaled(
			[u1, u2, 10], [g, 2.5, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2], LinRange(0, 80, 2000))
	...
	I2_color = :tomato1
	alphaAmount = 0.7
	lines!(ax6, T_traj, I1_traj, linewidth = 4, color = I2_color, alpha = 1.0)
	#lines!(ax6, T_traj, I2_traj, linewidth = 4, color = I1_color, alpha = 1.0, linestyle = :dash)

	# compute solution
	sol = integrateScaled(
			[u1, u2, 0.0], [g, 2.5, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2], LinRange(0, 80, 2000))
	...
	I2_color = :tomato1
	alphaAmount = 0.7
	#lines!(ax6, T_traj, I1_traj, linewidth = 4, color = I2_color, alpha = 0.5)

	Legend(contourfig[1, 2],
	    	[
				[LineElement(color = i, linewidth = 4, alpha = 1.0) for i in colors], 
				#[LineElement(color = I1_color, linewidth = 4, alpha = alphaAmount, linestyle = :dot), LineElement(color = I2_color, linewidth = 4, alpha = alphaAmount)]
			],
		
	    	[
				[string("I₁(0) = ", i) for i in vals],
				#["I₁(t)", "I₂(t) chronic"],
			],
	...
	#rowsize!(fig.layout, 2, Fixed(250))
	...
	label_size = 30
	text!(fig.scene, 0, 1, text = "A)", fontsize = label_size, font = :bold, space = :relative, offset = (0, -45))
	text!(fig.scene, 0, 1, text = "B)", fontsize = label_size, font = :bold, space = :relative, offset = (480, -45))
	text!(fig.scene, 0, 1, text = "C)", fontsize = label_size, font = :bold, space = :relative, offset = (1000, -45))
	text!(fig.scene, 0, 1, text = "D)", fontsize = label_size, font = :bold, space = :relative, offset = (480, -200))
	text!(fig.scene, 0, 1, text = "E)", fontsize = label_size, font = :bold, space = :relative, offset = (1000, -200))
	text!(fig.scene, 0, 1, text = "F)", fontsize = label_size, font = :bold, space = :relative, offset = (480, -350))
	text!(fig.scene, 0, 1, text = "G)", fontsize = label_size, font = :bold, space = :relative, offset = (1000, -350))
	text!(fig.scene, 0, 1, text = "H)", fontsize = label_size, font = :bold, space = :relative, offset = (1240, -45))

	save("exports/20241205_02_fig4.png", fig)
...
manifest_format = "2.0"
project_hash = "412ba418afdc53b08a4e98160620abe77a85b867"
