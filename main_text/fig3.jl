...
# ╔═╡ 308a4dfc-5dfb-4460-b61d-21ffd1e7c11a
chronicBlue = cgrad(:PuBu_3, 3; categorical = true)[2]

# ╔═╡ 742e0bdf-efaa-4f40-950c-12f384593398
acuteGrey = cgrad(:PuBu_3, 3; categorical = true)[1]

# ╔═╡ f425cd27-a4a7-49e0-af9f-8a5877dfd8fe
unstableLCColor = :blueviolet
...
function makePlotReduced(p, IC, fig)
	...
	rs = 10 .^ LinRange(-1, 1.5, 20)
	u02s = 10 .^ LinRange(-1, 3.5, 20)
	...
	ax1, hm = heatmap(
		fig[1, 1], 
		rs, u02s, mat,
		colormap = cgrad([acuteGrey, chronicBlue], 2; categorical = true),
		colorrange = (0.5, 2.5),
		nan_color = :white,
		axis=(;yscale=log10, 
				xscale = log10, 
	...
	plot!(ax1, [0.2, 6.0], [1, 1], color = :black)
...
function makeStreamPlotReduced(fig, params, (minX, maxX, minY, maxY); show_bassin=false, basinColor = [:aquamarine])
	...
	for i in getZeros(params, lims[1:2], lims[3:4])
		...
		if (sign(real(eigenValues[1])) < 0) && (sign(real(eigenValues[2])) < 0)	# attractor
			scatter!(rightPannelAx1, i[1], i[2], color = :black, markersize = 20)
		elseif (sign(real(eigenValues[1])) > 0) && (sign(real(eigenValues[2])) > 0)	# repelor
			scatter!(rightPannelAx1, i[1], i[2], color = :white, markersize = 20)
		else
			scatter!(rightPannelAx1, i[1], i[2], color = :black, markersize = 25, marker = :cross)
		end
	...
	tt = (0:0.01:50)
	sol = integrateSystemReduced([g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], [0.0, 10^0], tt)
	x = log10.(abs.(sol[1, :]))
    y = log10.(abs.(sol[2, :]))
...
function makeStreamPlotReduced2(fig, params, (minX, maxX, minY, maxY); show_bassin=false, basinColor = [:aquamarine], maxTime = 1000)
	...
	lines!(rightPannelAx1, Ttraj, Itraj, color = unstableLCColor, linewidth = 4, linestyle = :dash)

	# Plot example trajectory
	tt = (0:0.01:50)
	sol = integrateSystemReduced([g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], [0.0, 1.0], tt)
	x = log10.(abs.(sol[1, :]))
    y = log10.(abs.(sol[2, :]))
    c = LinRange(1, tt[end], length(x))
...
# ╔═╡ aaa1bc9e-8ae4-4d71-9b0e-b7e272e88fa8
function bifDiagram(fig)
	# define parameters
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = baseP
	...
	ax1 = Axis(fig[1, 1], ylabel = "T-Cells", xlabel = "Growth rate (r)", yticks = ([-4, -2, 0, 2, 4, 6], ["10⁻⁴", "10⁻²", "10⁰", "10²", "10⁴", "10⁶"]), xlabelsize = 20, ylabelsize = 20, xticklabelsize = 20, yticklabelsize = 20)
	ylims!(ax1, -4, 6)

	
	rs = LinRange(2.4, 12, 200)
	bassin = getBassinBounds(rs, baseP, [10^4, 10^2])
	band!(ax1, rs, bassin[1], bassin[3], alpha = 0.2, color = :grey60)
	lines!(ax1, rs, bassin[1], color = unstableLCColor, linewidth = 4, linestyle = :dash)
	lines!(ax1, rs, bassin[3], color = unstableLCColor, linewidth = 4, linestyle = :dash)


	rs = LinRange(0.0, 12, 50)
	tr = tracesFixedPoints(rs, baseP, (-4, 6, -4, 5))
	bassin = getBassinBounds(rs, baseP, [10^4, 10^2])
	lines!(ax1, rs, tr[:, 1, 1], linewidth = 4, linestyle = :dot, color = :black)
	lines!(ax1, rs, tr[:, 2, 1], linewidth = 4, color = :black)


	xpositions = [0.2, 2.4941, 10.0]
	xoffset = 0.2
	#names = ["E", "F", "G"]

	

	vlines!(ax1, xpositions, linewidth = 2, color = :black)
	...
	ax1.alignmode = Mixed(left = 0, right=0)
end

# ╔═╡ d78cf70c-b73f-44a9-bb09-8ee3eb8dd507
cgrad([:red, :green], 3; categorical = true)
...
let
	...
	subFig1 = fig[2:3, 1] = GridLayout()
	makePlotReduced(baseP, baseU0, fig[1, 2])


	runsFig = fig[2, 1:2] = GridLayout()

	showRuns(
		[g, 0.2, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], 
		[0.0, 1.0], 
		runsFig[1, 1]
	)

	showRuns(
		[g, 6.0, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], 
		[0.0, 1.0], 
		runsFig[1, 2]
	)
	
	rowsize!(runsFig, 1, Relative(0.8))

	bifDiagram(fig[4, 1:2])

	
	streamplotsFig = fig[3, 1:2] = GridLayout()


	plotBounds = (-4, 6.5, -1, 3.5)
	makeStreamPlotReduced(streamplotsFig[1, 1], [g, 0.2, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], plotBounds, show_bassin=true, basinColor = [:grey60])
	makeStreamPlotReduced2(streamplotsFig[1, 2], [g, 2.4941, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], plotBounds, show_bassin=true, basinColor = [:grey60])
	
	axstr = makeStreamPlotReduced2(streamplotsFig[1, 3], [g, 10.0, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], plotBounds, show_bassin=true, maxTime = 150, basinColor = [:grey60])
	...
	rowsize!(fig.layout, 1, 150)
	rowsize!(fig.layout, 4, 140)
	rowsize!(fig.layout, 3, 180)

	rowgap!(fig.layout, 40.0)

	save("makieExports/fig3.png", fig)
