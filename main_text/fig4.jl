...
# ╔═╡ d1be63e0-13da-11f0-2da8-4ddf5e44dd6b
using DifferentialEquations, GLMakie, LinearAlgebra, IntervalRootFinding, BenchmarkTools, ForwardDiff, PlutoUI, MakieExtra, Colors

# ╔═╡ d96e1d81-fa85-4951-b38a-7c18d8ba4fc7
begin
	include("landscapeClasses.jl")
	include("landscapeUtilities.jl")
end

# ╔═╡ 1a49a6df-6d73-4a66-9931-fefa3777733f
import Contour

# ╔═╡ f7605eb1-28da-49bd-8c53-f9a94ddc5d30
begin
	attractor = FixedPoint([-1 0; 0 -1], "attractor", :springgreen4, -1, 0)
	repellor = FixedPoint([1 0; 0 1], "repellor", :mediumblue, 1, 0)
	rightSpiral = FixedPoint([0 1; -1 0], "rightSpiral", :red, 0, 1)
	leftSpiral = FixedPoint([0 -1; 1 0], "leftSpiral", :deepskyblue2, 0, -1);
end

# ╔═╡ 6903bb99-7c39-4bb5-8ceb-4620fd07a877
md"# Landscape figure
This notebook is to create the figure for the landscape. I will use a central definition of the landscape used so that I can modify it later with a more appropriate version of it.
"

# ╔═╡ 275516c4-0285-4133-b0ec-e32f6aa39729
function makeImmuneLandscape(factor::Real)
	M1 = LandscapeModule([-4, -3], attractor, 6, 0.9)
	M2 = LandscapeModule([0, 0], repellor, 1.0, 2.5)
	M3 = LandscapeModule([0, 0], rightSpiral, 0.6, 3.0)
	M4 = LandscapeModule([0, 0], attractor, 3.3, 1.5)
	M5 = LandscapeModule([-6., -2.], rightSpiral, factor*0.5, 4.0)
	return Landscape(
		[M1, M2, M3, M4, M5], A0=0.01
	)
end

# ╔═╡ 7f105cbf-2495-4edc-b76b-85d14a444368
function shadingScaling(x)
	return -(x-1)^8+1
end

# ╔═╡ d64530d2-b68f-4e08-89a4-c6fad881b6db
begin
	stableFPColor = :chocolate1
	unstableFPColor = :springgreen4
	saddleFPColor = :royalblue
	LCColor = :blueviolet
	speedColormap = :diverging_bwr_55_98_c37_n256
end

# ╔═╡ 354bd42b-019c-4a46-a29c-c08668c9970a
[colorant"goldenrod1", colorant"mediumblue", colorant"red", colorant"deepskyblue2"]

# ╔═╡ 580aa8f2-5a5d-4bc4-a96c-6890da0f91a1
function makeAcuteLandscape(factor::Real)
	M1 = LandscapeModule([-4, -3], attractor, 6, 0.9)
	M2 = LandscapeModule([0, 0], repellor, 1.0, 2.5)
	M3 = LandscapeModule([0, 0], rightSpiral, 0.6, 3.0)
	M5 = LandscapeModule([-6., -2.], rightSpiral, factor*0.5, 4.0)
	return Landscape(
		[M1, M2, M3, M5], A0=0.01/2
	)
end

# ╔═╡ 74c6294a-9a75-4392-9b94-e3fc8200e43a
function makeChronicLandscape(factor::Real)
	M4 = LandscapeModule([0, 0], attractor, 3.3, 1.5)
	return Landscape(
		[M4], A0=0.01/2
	)
...
# ╔═╡ a80ce1c4-ca2e-4ebf-8d89-3e5052e6c225
begin
	# Here we make the components to add for the landscape
	ls1 = makeAcuteLandscape(0.2)
	ls2 = makeChronicLandscape(0.2)
	ls3 = makeImmuneLandscape(0.2)
end

# ╔═╡ 56465eee-da67-412c-8549-8bd94c1de596
function landscapeRotationalPotential(landscape::Landscape, x::Vector{Float64})
    # function to compute the potential at a given point
    return sum([LM.weight * LM.fpType.rotationSign * exp(-customNorm(LM.x - x)^2 / (2 * LM.sigma^2)) for LM in landscape.moduleList])
end

# ╔═╡ c50bc3b0-fd72-4597-975f-ea840365a917
function takeGrad(ls, x)
	# function to get gradient of function
	dx1 = ForwardDiff.derivative(x1->landscapeRotationalPotential(ls, [x1, x[2]])[1], x[1])
	dx2 = ForwardDiff.derivative(x2->landscapeRotationalPotential(ls, [x[1], x2])[1], x[2])

	grad = [dx1, dx2]
	antiGrad = cross([dx1, dx2, 0], [0, 0, 1])[1:2]

	return grad/10, -antiGrad/1000
end

# ╔═╡ 331a65ce-9c4c-4bf8-b394-feec8cd21066
function plotPotential(ls::Landscape, fig; axis_kw...)
    ...
    ax = Axis3(
		...
		elevation = 0.12*2*π,
		xlabel = "T-Cells",
		ylabel = "Immune Challenge",
		zlabel = "Potential",
		zticks = (colrange .+offset, string.(colrange)),
		zlabeloffset = 30,
		xlabeloffset = 20,
		ylabeloffset = 20,
	...
	zlims!(ax, minimum(colrange)+offset, 7+offset)
	...
	contour!(ax, 
			 minX:0.1:maxX, 
			 minY:0.1:maxY, 
			 (x,y)->landscapePotential(ls, [x, y]),
			 colormap = colmap,
			 colorrange = (minimum(colrange), maximum(colrange)),
			 levels = 10
			)
	...
	surface!(ax, 
			 minX:0.1:maxX, 
			 minY:0.1:maxY, 
			 (x,y)->landscapePotential(ls, [x, y])+offset,
			 colormap = colmap,
			 colorrange = (minimum(colrange), maximum(colrange)) .+offset,
			)

	# Make the contour plots for the rotational part
	
	x = minX:0.01:maxX
	y = minY:0.01:maxY
	z1 = [Float64(landscapeRotationalPotential(ls, [xi, yi])) for xi in x, yi in y]
	
	for cl in Contour.levels(Contour.contours(x, y, z1, 5))
	    ...
	    for line in Contour.lines(cl)
			...
			z = [landscapePotential(ls, [xi, yi]) + offset+0.01 for (xi, yi) in zip(xs, ys)]
			#z = [1 for (xi, yi) in zip(xs, ys)]
			
			#println(size(z))
			#println(size(x), size(y))
	    	lines!(ax, xs, ys, z, linewidth = 1, color = :white)
			arrowLocation = [xs[1], ys[1]]
			grad, antigrad = takeGrad(ls, arrowLocation)
			arrows!(ax, xs[1:1], ys[1:1], z[1:1], [antigrad[1]], [antigrad[2]], [0], arrowcolor = :white, linecolor = :white, arrowsize = Vec3f(0.1, 0.1, 0.1))
	    end
	end
...
# ╔═╡ 4fcafd97-c143-406b-961f-e62fedf916b0
function plotRotationalPotential(ls::Landscape, fig; axis_kw...)
    # Function to plot (with Makie) the landscape
    ax = Axis3(
        fig[1, 1];
		azimuth = 2*π*0.7,
	...
	surface!(ax, 
			 minX:0.1:maxX, 
			 minY:0.1:maxY, 
			 (x,y)->landscapeRotationalPotential(ls, [x, y]),
			 colormap = :ice
			)
...
# ╔═╡ 4f46680c-3d4f-4ceb-a75a-cd52830241b0
function integrateTrajectory(landscape::Landscape, IC::Vector{Float64}, times)
	function f(du, u, p, t)
		du[1], du[2] = makeDifferentialSystem(landscape, u)
	end

	# Define ode problem
	prob = ODEProblem(f, IC, (times[1], times[end]), ())
	# solve problem
	sol = solve(prob, Rodas4(), saveat = times);

	return sol
end

# ╔═╡ 8b0ddd2e-6814-4c91-bc7f-a45a2ea404da
function findFixedPoints(ls::Landscape)
	# function to find the fixed points and limit cycle of the system
	stableFP = []
	unstableFP = []
	saddleFP = []
	LCBoundaries = []
	...
	sol = integrateTrajectory(ls, [-0.1,0.1], -(0:0.1:500))
	if abs(sol[1, end]) < 6.0
		push!(LCBoundaries, [minimum(sol[1, end-200:end]), maximum(sol[1, end-200:end])])
	else
		push!(LCBoundaries, [NaN, NaN])
	end

	return stableFP, unstableFP, saddleFP, LCBoundaries

end

# ╔═╡ bf29b12c-8db0-4caa-be82-82df303eb8d8
function bifurcationDiagram(fig; snapshots = [])
	# Function to generate the plot for the bifurcation diagram
	ax = Axis(fig[1,1], xlabel = "Speed Parameter", ylabel = "Fixed points (T-Cells)", xreversed = true, yticklabelsvisible = false, yticksvisible = false)
	...
	factors = 0:0.01:1.0 #0:0.01:1.0
	
	for factor in factors
		stableFP, unstableFP, saddleFP, LCBoundaries = findFixedPoints(makeImmuneLandscape(factor))
		#println(saddleFP)
		push!(stable1, maximum(stableFP))
		push!(stable2, minimum(stableFP))
		push!(saddle, saddleFP[1])
		push!(LCtop, maximum(LCBoundaries[end]))
		push!(LCbot, minimum(LCBoundaries[end]))
	end

	# convert to float array for the band plotting
	LCbot = convert(Vector{Float64}, LCbot)
	LCtop = convert(Vector{Float64}, LCtop)
	lines!(ax, factors, LCtop, linewidth = 4, color = LCColor, linestyle = :dash, label = "Unstable LC")
	lines!(ax, factors, LCbot, linewidth = 4, color = LCColor, linestyle = :dash)
	band!(ax, factors, LCbot, LCtop, alpha = 0.2, color = :grey60)
	
	lines!(ax, factors, stable1, linewidth = 4, color = :black)
	lines!(ax, factors, stable2, linewidth = 4, color = :black, label = "Stable FP")
	lines!(ax, factors, saddle, linewidth = 4, label = "Saddle FP", color = :black, linestyle = :dot)


	vlines!(ax, snapshots, linewidth = 2, color = :black)

	Legend(fig[1, 2], 
		   [LineElement(color = LCColor, linestyle = :dash, linewidth = 4), 
		   	LineElement(color = :black, linestyle = :solid, linewidth = 4),
		   	LineElement(color = :black, linestyle = :dot, linewidth = 4),
			MarkerElement(marker=:circle, markersize=15, color=:black),
			MarkerElement(marker=:circle, markersize=15, color=:white, strokewidth=1, strokecolor=:black),
			MarkerElement(marker=:cross, markersize=15, color=:black),
			LineElement(color=:black, linestyle=:dash)
		   ],
		   ["Unstable LC", "Stable FP", "Saddle", "Stable FP",  "Unstable FP", "Saddle", "Nullclines"],
		   tellwidth = true, 
		   halign = :left, 
		   valign = :top,
		   margin = (10, 10,10,10),
...
# ╔═╡ 23bb794e-ff30-4833-9827-95cfec1ddd62
function plotStreamplot2(ls::Landscape, fig, i2; axis_kw...)
    # Function to plot (with Makie) the landscape
    ax = Axis(
        fig[1, 1], aspect=1;
        axis_kw...
	...
	# Make trajectories
	sol1 = integrateTrajectory(ls, [-2,0.], 0:0.1:40)
	sol2 = integrateTrajectory(ls, [i2,0.], -(0:0.1:40))

	lines!(ax, sol1[1, :], sol1[2, :], linewidth = 4)
	lines!(ax, sol2[1, :], sol2[2, :], linewidth = 4)
...
# ╔═╡ a1aefd09-6446-408c-b8ad-e5aa7ccb4ad1
@bind x PlutoUI.Slider(0:0.01:1, show_value=true)

# ╔═╡ 267edab0-a220-4359-a53a-50102ea5c742
@bind y1 PlutoUI.Slider(-4:0.001:0, show_value=true)

# ╔═╡ 4d6307f1-fb4e-4e93-9128-8323580e0d2f
function isacute(factor, Yinitial)
    # function to test if the landscape's trajectory is acute

    # initialize landscape
    ls = makeImmuneLandscape(factor)

    # integrate the system 
    traj = integrateTrajectory(ls, [-4., Yinitial], 0:0.05:60)
...
# ╔═╡ d84a3251-765e-4abc-a053-16c40a8ad49f
function makeBassins(ls)
	xs = LinRange(-6, 6, 40)
	ys = LinRange(-6, 6, 40)
	...
	for (i, x) in enumerate(xs)
		for (j, y) in enumerate(ys)
		    ...
		    # integrate the system 
		    traj = integrateTrajectory(ls, [x, y], 0:0.1:400)
			lastVal = traj[2, end]
...
# ╔═╡ 1952cf8c-3fcc-4c8e-b6b9-86b6d71b8bf5
function plotStreamplot(ls::Landscape, fig; showChronic = true, showAcute= true, showLC = true, bassinColor = :grey40, axis_kw...,)
    # Function to plot (with Makie) the landscape
    ax = Axis(
        fig[1, 1], aspect = 1, xticklabelsvisible = false, yticklabelsvisible = false, yticksvisible = false, xticksvisible = false;
        axis_kw...
    )
	...
	# Make attraction bassin
	bassinX, bassinY, bassinMap = makeBassins(ls)
	heatmap!(ax, bassinX, bassinY, bassinMap, colormap = [bassinColor], alpha = 0.2)
	...
	# Make trajectories
	colmap = speedColormap
	if showChronic
		sol1 = integrateTrajectory(ls, [-2,0.], 0:0.1:40)
		lines!(ax, sol1[1, :], sol1[2, :], linewidth = 4, color = 2, colorrange = (1, 2), colormap = colmap)
	end

	if showAcute
		sol2 = integrateTrajectory(ls, [-4,0.], 0:0.1:40)
		lines!(ax, sol2[1, :], sol2[2, :], linewidth = 4, color = 1, colorrange = (1, 2), colormap = colmap)
	end

	if showLC
		# Plot limit cycle
		solLC = integrateTrajectory(ls, [0.1,0.1], -(0:0.1:400))
		#println(size(solLC[:, :]))
		lines!(ax, solLC[1, 50:250], solLC[2, 50:250], color = LCColor, linewidth = 4, linestyle = :dash)
	end
...
# ╔═╡ 44dc6a6c-6ff5-4830-9f9a-a308186ad05c
function speedTrajectoryFig(fig)
	# function to plot the trajectory that illustrate the impact of the growth rate on the slope of the time series

	times = 0:0.01:35

	ax1 = Axis(fig[1, 1], xlabel = "Time", ylabel = "Immune Challenge", limits = (-0.1, maximum(times), -6, 6), yticklabelsvisible = false, yticksvisible = false)

	ax2 = Axis(fig[1, 2], xlabel = "Time", limits = (-0.1, 5, -1, 5), yticklabelsvisible = false, yticksvisible = false)

	ll = 8
	factors = LinRange(0, 1, ll)
	colmap = speedColormap
	
	for (i, factor) in enumerate(factors)
		sol = integrateTrajectory(makeImmuneLandscape(factor), [-4.0, 0.0], times)

		
		lines!(ax1, times, sol[2, :], color = i, colorrange = (1, ll), colormap = colmap, linewidth = 4, alpha=0.8)

		lines!(ax2, times, sol[2, :], color = i, colorrange = (1, ll), colormap = colmap, linewidth = 4, alpha = 0.8)
	...
	Colorbar(fig[1, 3], limits = (0, 1), colormap = colmap, label = "Speed Parameter")
...
# ╔═╡ 4c5e27c0-d144-43c9-ad67-d8e42df1f81a
function phaseDiagramFig(fig)
	ax = Axis(fig[1, 1], xlabel = "Speed Parameter", ylabel = "Initial challenge", xreversed = true, yticksvisible = false, yticklabelsvisible = false)

	factors = LinRange(0, 1, 21)
	initialValues = LinRange(-2, 5, 21)

	dat = fill(NaN, size(factors)..., size(initialValues)...)

	for (i, factor) in enumerate(factors)
		for (j, IC) in enumerate(initialValues)
			dat[i, j] = isacute(factor, IC)
		end
	end

	#println(dat)

	heatmap!(ax, factors, initialValues, dat, colormap = speedColormap)

	return fig

end

# ╔═╡ a718647b-b73a-44eb-9d39-05f768b54ca5
function plotLandscape(ls::Landscape, fig, title; showAcute = true, showChronic = true, axis_kw...)
    # Function to plot (with Makie) the landscape
    ax = Axis(
        fig[1, 1], aspect=1;
        axis_kw...
    ...
    for LM in ls.moduleList
        scatter!(
            ...
            markersize=LM.sigma * 2.35,
            markerspace=:data, marker=Circle, color=LM.fpType.color,
            alpha=0.7 *shadingScaling(LM.weight / ls.maxWeight),
            strokewidth=0)
    end
	...
	colmap = speedColormap
	if showChronic
		sol1 = integrateTrajectory(ls, [-2,0.], 0:0.1:80)
		lines!(ax, sol1[1, :], sol1[2, :], linewidth = 4, color = 2, colorrange = (1, 2), colormap = colmap)
	end

	if showAcute
		sol2 = integrateTrajectory(ls, [-4,0.], 0:0.1:80)
		lines!(ax, sol2[1, :], sol2[2, :], linewidth = 4, color = 1, colorrange = (1, 2), colormap = colmap)
	end
...
# ╔═╡ c920d05c-75ce-454d-8008-c194e26f24bf
let
	fig = Figure(size = (1520, 645), padding = 300)
	
	
	# Fig for adding the subplots together
	limitsSurface = (-6, 6, -6, 6)
	#limitsSurface = (-5, 5, -5, 5)
	additionSubfig = fig[1:2, 1] = GridLayout()
	#Label(additionSubfig[0, 1:3], " ")
	plotLandscape(ls1, additionSubfig[1, 1], "", showChronic = false, limits = limitsSurface, xlabel = "T-Cells", ylabel = "Immune Challenge", xticklabelsvisible = false, yticklabelsvisible = false, xticksvisible = false, yticksvisible = false)
	#Label(additionSubfig[1, 2], "+", fontsize = 60, tellheight = false)
	plotLandscape(ls2, additionSubfig[1, 2],"", showAcute = false, limits = limitsSurface, xlabel = "T-Cells", ylabel = "Immune Challenge", xticklabelsvisible = false, yticklabelsvisible = false, xticksvisible = false, yticksvisible = false)
	#Label(additionSubfig[1, 4], "=", fontsize = 60, tellheight = false)
	plotLandscape(ls3, additionSubfig[1, 3],"", limits = limitsSurface, xlabel = "T-Cells", ylabel = "Immune Challenge", xticklabelsvisible = false, yticklabelsvisible = false, xticksvisible = false, yticksvisible = false)
	
	plotPotential(ls1, additionSubfig[2, 1], limits = limitsSurface)
	#Label(additionSubfig[2, 2], "+", fontsize = 60, tellheight = false)
	plotPotential(ls2, additionSubfig[2, 2], limits = limitsSurface)
	#Label(additionSubfig[2, 4], "=", fontsize = 60, tellheight = false)
	plotPotential(ls3, additionSubfig[2, 3], limits = limitsSurface)
	colgap!(additionSubfig, 30)

	#=
	plotRotationalPotential(ls1, additionSubfig[3, 1], limits = limitsSurface)
	#Label(additionSubfig[3, 2], "+", fontsize = 60, tellheight = false)
	plotRotationalPotential(ls2, additionSubfig[3, 2], limits = limitsSurface)
	#Label(additionSubfig[3, 4], "=", fontsize = 60, tellheight = false)
	plotRotationalPotential(ls3, additionSubfig[3, 3], limits = limitsSurface)=#


	# Figs for the bifurcation diagram
	bifurcationSubfig = fig[1, 2] = GridLayout()

	# Main bifurcation plot
	snapshots = [0.0, 0.355, 1.0]
	bifurcationDiagram(bifurcationSubfig[1, 1:3], snapshots=snapshots)

	# Bifurcation snapshots
	plotStreamplot(
		makeImmuneLandscape(snapshots[3]), 
		bifurcationSubfig[2, 1], showLC=false, showAcute = false, limits = limitsSurface, xlabel = "T-Cells", ylabel = "Immune Challenge")
	plotStreamplot(
		makeImmuneLandscape(snapshots[2]), 
		bifurcationSubfig[2, 2], limits = limitsSurface, yticklabelsvisible = false, xlabel = "T-Cells")
	plotStreamplot(
		makeImmuneLandscape(snapshots[1]), 
		bifurcationSubfig[2, 3], limits = limitsSurface, yticklabelsvisible = false, xlabel = "T-Cells")


	# Speed characterization
	speedSubfig = fig[2, 2] = GridLayout()
	speedTrajectoryFig(speedSubfig[1, 1:2])
	phaseDiagramFig(speedSubfig[1, 3])
	#Axis(speedSubfig[1, 3])
	colgap!(speedSubfig, 30)
	rowsize!(fig.layout, 2, Relative(0.25))
	colsize!(fig.layout, 1, Relative(0.6))


	save("makieExports/fig4.png", fig)
	fig

end

# ╔═╡ b7defa5c-d4b9-4afc-942c-48aaa077301c
let 
	fig = Figure()
		limitsSurface = (-6, 6, -6, 6)
	plotStreamplot2(
		makeImmuneLandscape(x), 
		fig[1, 1], y1, limits = limitsSurface, xlabel = "T-Cells", ylabel = "Immune Challenge")
	fig
end

# ╔═╡ 7d24eadc-d165-473c-8a62-fcfc92af06a6
# ╠═╡ disabled = true
#=╠═╡
let
	fig = Figure(size = (1520, 800))
	
	
	# Fig for adding the subplots together
	limitsSurface = (-6, 6, -6, 6)
	additionSubfig = fig[1:2, 1] = GridLayout()
	plotLandscape(ls1, additionSubfig[1, 1], limits = limitsSurface, xlabel = "T-Cells", ylabel = "Immune Challenge")
	Label(additionSubfig[1, 2], "+", fontsize = 60, tellheight = false)
	plotLandscape(ls2, additionSubfig[1, 3], limits = limitsSurface, xlabel = "T-Cells", ylabel = "Immune Challenge")
	Label(additionSubfig[1, 4], "=", fontsize = 60, tellheight = false)
	plotLandscape(ls3, additionSubfig[1, 5], limits = limitsSurface, xlabel = "T-Cells", ylabel = "Immune Challenge")
	
	plotPotential(ls1, additionSubfig[2, 1], limits = limitsSurface)
	Label(additionSubfig[2, 2], "+", fontsize = 60, tellheight = false)
	plotPotential(ls2, additionSubfig[2, 3], limits = limitsSurface)
	Label(additionSubfig[2, 4], "=", fontsize = 60, tellheight = false)
	plotPotential(ls3, additionSubfig[2, 5], limits = limitsSurface)

	plotRotationalPotential(ls1, additionSubfig[3, 1], limits = limitsSurface)
	Label(additionSubfig[3, 2], "+", fontsize = 60, tellheight = false)
	plotRotationalPotential(ls2, additionSubfig[3, 3], limits = limitsSurface)
	Label(additionSubfig[3, 4], "=", fontsize = 60, tellheight = false)
	plotRotationalPotential(ls3, additionSubfig[3, 5], limits = limitsSurface)
	...
	# Main bifurcation plot
	bifurcationDiagram(bifurcationSubfig[1, 1:3])

	# Bifurcation snapshots
	plotStreamplot(
		makeImmuneLandscape(0.0), 
		bifurcationSubfig[2, 1], limits = limitsSurface, xlabel = "T-Cells", ylabel = "Immune Challenge")
	plotStreamplot(
		makeImmuneLandscape(0.3885), 
		bifurcationSubfig[2, 2], limits = limitsSurface, xlabel = "T-Cells", ylabel = "Immune Challenge")
	plotStreamplot(
		makeImmuneLandscape(1.0), 
		bifurcationSubfig[2, 3], limits = limitsSurface, xlabel = "T-Cells", ylabel = "Immune Challenge")


	# Speed characterization
	speedSubfig = fig[2, 2] = GridLayout()
	speedTrajectoryFig(speedSubfig[1, 1:2])
	#speedTrajectoryFig(speedSubfig[1, 1], colmap = :linear_blue_5_95_c73_n256, times = 0:0.01:35)
	#speedTrajectoryFig(speedSubfig[1, 2], colmap = :linear_blue_5_95_c73_n256, times = 0:0.01:6, ylabel = "", colbar=true)
	phaseDiagramFig(speedSubfig[1, 3])

	rowsize!(fig.layout, 2, Relative(1/3))


	save("exports/20250411_fig_landscape.png", fig)
	fig
...
manifest_format = "2.0"
project_hash = "d4b995e593bfacf8c189490d036f1582ab97e6c7"
...
git-tree-sha1 = "b306944ae65a88a0ce53ff744202def7144995bc"
...
git-tree-sha1 = "5738c8b3eabe75ba4c96a15e6037a10bf519138e"
