...
# ╔═╡ 9811d740-a912-11ef-13c0-7962a67f8749
using DifferentialEquations, CairoMakie, FileIO

# ╔═╡ 71c4b985-7559-4065-960d-03c346c0df9f
custom_linewidth = 4

# ╔═╡ ae44f919-a234-4ec1-8954-431bb18e35d4
function RELU(x)
	return max(0, x)
end

# ╔═╡ 6924bce8-7387-49f1-9ac3-d2b575073740
...
# ╔═╡ e5000452-e511-4443-a605-bfdd2048abc7
function diffEquationFader(du, u, p, t)
...
# ╔═╡ ca72e5be-54af-4b88-b2ac-066c5beb84ab
function integrateFader(p, u0, times)
	prob = ODEProblem(diffEquationFader, u0, (times[1], times[end]), p);
	sol = solve(prob, AutoTsit5(Rosenbrock23()), saveat = times[2]-times[1]);
	return sol
end

# ╔═╡ e70a037d-18a7-4fc0-a7c8-188f07e31d1d
function showRuns(p, u0, fig; visiblex = true, visibley1 = true, visibley2 = true)

	# times for simulation
	times = LinRange(0, 20, 2000)

	# compute solution
	sol = integrateFader(p, u0, times)
	...
	ax1 = Axis(
		...
		xlabel = "Time",
		yscale = log10,
		yticklabelsize = ysize,
		xticklabelsize = xsize,
		ylabelsize = ysize,
		xlabelsize = xsize,
		ylabelcolor = I_color,
		xlabelpadding = -5,
		ylabelpadding = -5,
		xlabelvisible = visiblex,
		xticklabelsvisible = visiblex,
		ylabelvisible = visibley1,
		yticklabelsvisible = visibley1,
		#yticks = ([10^-2, 10^0,  10^2, 10^4, 10^6], ["10⁻²", "10⁰", "10²", "10⁴", "10⁶"]),
		
		#title = string("r = ", p[2])
	)
	
	ax2 = Axis(fig[1, 1], yticklabelcolor = T_color, ylabel = "T-Cells", yscale = log10, ygridvisible = false,  yticklabelsize = ysize, ylabelsize = ysize,ylabelcolor = T_color, 
		#yticks = ([10^-2, 10^0,  10^2, 10^4], ["10⁻²", "10⁰", "10²", "10⁴"]), 
		ylabelvisible = visibley2,
		yticklabelsvisible = visibley2, ylabelpadding = -5,)
	
	xlims!(ax1, 0, times[end])
	xlims!(ax2, 0, times[end])
	ylims!(ax2, 0.005, 7000000)
	ylims!(ax1, 0.005, 20000)
	lines!(ax1, times, I_traj, color = I_color, linewidth = custom_linewidth)
	lines!(ax2, times, T_traj, color = T_color, linewidth = custom_linewidth)
...
# ╔═╡ fd53f917-ad26-4ecf-93eb-dfa829eb9097
baseP = (
	g = 10.0,
	r = 0.2,
	T_star = 100.0,
...
# ╔═╡ 8b73dda9-d138-4ebb-88ff-7d41a343d72b
baseU0 = (
	T = 0.0,
	I = 1.0,
	Treg = 1e4,
...
# ╔═╡ f4aa6726-6259-4b69-b341-688eca9060ed
function initialQuantityExp(r, u0; delay = 1)
	# function to compute the amount of pathogens if grown exponentially after a time
	return u0*exp(r*delay)
end

# ╔═╡ bc37b8ce-4b08-408b-abdf-2ee19a6ff483
function makePlotFader(p, IC, fig)
	...
	rs = 10 .^ LinRange(-1, 1.5, 150)
	u02s = 10 .^ LinRange(-1, 3.5, 150)
	...
	plot!(ax1, [0.2, 2.0, 6.0, 0.2], [1, 1, 1, 2000], color = :black)

	#text!(ax1, 0.2, 7, text = "D", fontsize = 20, align = (:center, :top))
	#text!(ax1, 2.0, 7, text = "E", fontsize = 20, align = (:center, :top))
	#text!(ax1, 6.0, 7, text = "F", fontsize = 20, align = (:center, :top))

	# make isolines for initial quantity

	vlines!([4.5], linewidth = 4, color=:black)

	contour!(ax1, rs, u02s, initialQuantityExp, color = :red, levels = [10^-1, 10^0, 10^1, 10^2, 10^3, 10^4], 
		#labels = ([10^-1, 10^0, 10^1, 10^2, 10^3, 10^4], ["10⁻¹", "10⁰", "10¹", "10²", "10³", "10⁴"]), 
		labels = true, 
		linestyle = :dot, linewidth = 4, labelsize = 20)
...
# ╔═╡ 93d0c70f-d221-43e4-a7b8-dfa94344015c
let
	fig = Figure(size = (900, 1400), figure_padding = 0)

	#fig = f[1, 1] = GridLayout()

	
	#img = load(assetpath(string(pwd(), "/inkscape/fig1_network.png")))
	...
	hidespines!(ax_A)
	rowsize!(fig.layout, 1, 420)
	


	subFig1 = fig[3, 1:2] = GridLayout()

	#makePlotJump(baseP, baseU0, subFig1[1, 1])
	ax = makePlotFader(baseP, baseU0, subFig1[1, 1])
	colsize!(subFig1, 1, Aspect(1, 1.5))

	#Box(subFig1[2, 1], color = (:red, 0.2), strokewidth = 0)
	#Box(subFig1[1, 1], color = (:green, 0.2), strokewidth = 0)
	#ax.aspect = 1.0

	runfig = fig[1, 2] = GridLayout(alignmode = Outside(10))
	...
	showRuns(
		...
		[0, 1.0, 1e4, 0.0], 
		fig[2, 2]#, visibley1=false
	)
	
	runsubfig = fig[3, 1:2] = GridLayout()
	showRuns(
		[g, 0.2, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], 
		[0, 1.0, 1e4, 0.0], 
		fig[4, 1]#, visibley2=false
	)
	
	showRuns(
		[g, 2.0, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], 
		[0, 1.0, 1e4, 0.0], 
		fig[4, 2]#, visibley1=false, visibley2=false
	)
	...
	# Labels
	label_size = 35
	text!(fig.scene, 0, 0.95, text = "A)", fontsize = label_size, font = :bold, space = :relative, offset = (0, -0), align = (:left, :top))
	text!(fig.scene, 0., 0.35, text = "B)", fontsize = label_size, font = :bold, space = :relative, offset = (0, -0), align = (:left, :top))
	#text!(fig.scene, 0.03, 0.55, text = "C)", fontsize = label_size, font = :bold, space = :relative, offset = (0, -0), align = (:left, :top))
	
	#text!(fig.scene, 0.03, 0.26, text = "D)", fontsize = label_size, font = :bold, space = :relative, offset = (0, -0), align = (:left, :top))
	
	#text!(fig.scene, 0.53, 0.26, text = "E)", fontsize = label_size, font = :bold, space = :relative, offset = (0, -0), align = (:left, :top))
	#text!(f.scene, 0, 200, text = "C)", fontsize = label_size, font = :bold)
	#text!(f.scene, 350, 440, text = "D)", fontsize = label_size, font = :bold)
	#text!(f.scene, 350, 280, text = "E)", fontsize = label_size, font = :bold)
	#text!(f.scene, 350, 130, text = "F)", fontsize = label_size, font = :bold)

	save("makieExports/fig2.png", fig)
	fig
end
...
manifest_format = "2.0"
project_hash = "2dff2bd3d1ed5d71ecbb007f08898978c65f9a54"
