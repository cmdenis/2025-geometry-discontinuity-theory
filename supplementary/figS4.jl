...
# ╔═╡ 36c5de8c-a6b0-11ef-32c7-af4acb731efe
using DifferentialEquations, CairoMakie, PlutoUI

# ╔═╡ 69b91c99-8fac-41f9-a8e9-91c1879f2f0c
md"# Full system plots

 In this notebook, we make the plots for the full system"

# ╔═╡ 22191c96-8782-4b38-9d34-406df48f882b
function RELU(x)
	return max(0, x)
end

# ╔═╡ c447c188-83cd-4524-994b-29214c89a0bc
function convertParam(p)
    # function to convert the original parameters to the c1, c2, c3 parameters
    g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = p
...
# ╔═╡ 5b9d8afe-babc-4500-a9d4-f5632638db7c
...
# ╔═╡ 567e92dc-6b1c-404f-9954-4ea0e2f80e75
...
# ╔═╡ 8ea213cf-1c5b-4294-9890-8b4a0478b5d3
function diffEquationReduced(du, u, p, t)
	du[1], du[2] = fReduced(u[1], u[2], p)
end

# ╔═╡ 5541225c-77d5-4cff-a51c-f5554ee2a6a8
...
# ╔═╡ 354b3035-0d7e-4403-bdb0-fcb49eabeb7a
...
# ╔═╡ bcf49273-3d5d-4d3c-a203-88f6f9e7b8c7
...
# ╔═╡ 0a61f53a-5af1-4f9e-922d-4432db3958a3
...
# ╔═╡ a0f7f82e-a570-49b9-bdd8-29502ef43238
function fQuasistatic(du, u, p, t)
	du[1], du[2] = diffEquationQuasistatic(u[1], u[2], p)

	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = p
	
	du[3] = 2000*(Treg(T_star_reg, beta, delta2, mu, u[1], alpha) - u[3])
	du[4] = 2000*(il2(T_star_reg, beta, delta2, mu, u[1], alpha) - u[4])
end

# ╔═╡ 6343cc18-5368-4004-b4bf-9814996db92d
...
# ╔═╡ 2d161905-352e-468a-82e9-dbcc49a6e336
function diffEquationFull(du, u, p, t)
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = p

	# T-cells
	du[1] = kappa*u[2]*(T_star + alpha/delta1*(u[1]*u[4]/u[3])) - gamma*u[1]

	# infected cells
	n = 4
	k = 1
	du[2] = (g + r*u[2] - kappa*u[2]*(u[1] + T_star))*crossfader(u[2], n, k) - u[2]*(1-crossfader(u[2], n, k))
...
# ╔═╡ 84e1d663-4a5d-4c35-811b-01dd396f9577
md"Max time: $@bind maxTime NumberField(0:200., default=50.)"

# ╔═╡ b09d3212-4683-454c-9fab-a5342d872762
times = 0:0.01:maxTime

# ╔═╡ ec6fb918-a705-42e0-9168-0c3dd8e6a225
function integrateFull(p, u0, times)
	prob = ODEProblem(diffEquationFull, u0, (times[1], times[end]), p);
	sol = solve(prob, Rodas4P(), saveat = times[2]-times[1], abstol=1e-9, reltol=1e-6);
	return sol
end

# ╔═╡ 9375ed33-9af0-407d-9e38-56abbca375df
function showRuns(p, u0, fig)
	...
	# compute solution
	sol1 = integrateFull(p, u0, times)
	sol2 = integrateSystemQuasistatic(p, u0, times)
	sol3 = integrateSystemReduced(p, u0, times)
	...
	T_color = :dodgerblue2
	Treg_color = :purple1
	IL2_color = :springgreen3

	ax1 = Axis(fig[1, 1], xticklabelsvisible = false, ylabel = "Infected\nCells", yscale = log10)
	ax2 = Axis(fig[2, 1], xticklabelsvisible = false, ylabel = "T-Cells", yscale = log10)
	ax3 = Axis(fig[3, 1], xticklabelsvisible = false, ylabel = "Treg", yscale = log10)
	ax4 = Axis(fig[4, 1], xlabel = "Time (days)", ylabel = "IL2")
	...
	for (sol, color) in zip(sols, colors)
		...
		lines!(axes[1], times, I_traj, linewidth = 4, color = color, alpha = alpha_val)
		lines!(axes[2], times, T_traj, linewidth = 4, color = color, alpha = alpha_val)
		for i in 3:size(sol)[1]
			lines!(axes[i], times, sol[i, :], linewidth = 4, alpha = alpha_val, color = color)
		end
	end
...
# ╔═╡ 6b5d102f-0463-4d03-8aef-1574a5a21498
baseP = (
	g = 10.0,
	r = 0.2,
	T_star = 100.0,
...
# ╔═╡ a5aa3617-deae-4af0-87ae-31f5999b2db5
baseU0 = (
	T = 0.0,
	I = 1.0,
	Treg = 1e4,
...
# ╔═╡ 892b4ece-dffe-45e6-bbe0-c06472c15a55
let 
	fig = Figure(size = (1600, 600))

	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = baseP

	showRuns([g, 0.2, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], [baseU0...], fig[1, 1])
	
	showRuns([g, 2, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], [baseU0...], fig[1, 2])

	showRuns([g, 6, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], [baseU0...], fig[1, 3])

	elem_1 = LineElement(color = :blue, linestyle = nothing, linewidth = 4)
	elem_2 = LineElement(color = :orange, linestyle = nothing, linewidth = 4)
	elem_3 = LineElement(color = :green, linestyle = nothing, linewidth = 4)

	Legend(fig[1, 4],
	    [elem_1, elem_2, elem_3],
	    ["Full System", "Quasistatic Approximation", "Reduced System"],
	    patchsize = (35, 35), rowgap = 10)

	save("makieExports/figS4.png", fig)

	fig
end

# ╔═╡ f4d95410-a590-4faa-b2cf-a2f63d6196ef
function makePlotJump(p, IC, fig)
	...
	rs = 10 .^ LinRange(-1, 1.5, 100)
	u02s = 10 .^ LinRange(-1, 3.5, 100)
...
# ╔═╡ 8f9b1b28-1134-4f62-b699-1b0a512bf14a
function makePlotFader(p, IC, fig)
	# function to plot the result of infections
	...
	plot!(ax1, [0.2, 2.0, 6.0], [1, 1, 1], color = :black)
...
# ╔═╡ 63fcd82d-8492-4331-aaff-7aa23a13902f
let
	fig = Figure(size = (900, 500), figure_padding = 0)

	subFig1 = fig[1, 1] = GridLayout()

	makePlotJump(baseP, baseU0, subFig1[1, 1])
	makePlotFader(baseP, baseU0, subFig1[2, 1])

	runfig = subFig1[1:2, 2:3] = GridLayout()
	...
	showRuns(
		[g, 2.0, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], 
		[1e-3, 1.0, 1e4, 0.0], 
		runfig[2, 1]
	)
	showRuns(
