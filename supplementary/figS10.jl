...
# ╔═╡ e27fd042-e59a-4141-bb58-b330d3bc5424
function RELU(x)
	return max(0, x)
end

# ╔═╡ fdacf940-704d-469e-bfeb-7e4d4a3d1648
...
# ╔═╡ a5ebf0f6-5b07-474b-9a4d-524351f9da35
function fReduced(x, y, p)
    ...
    du2 = 1 + yp * (c1 - xp) # infected cells 
	[du1, du2]
end


# ╔═╡ 3da86139-6971-4a8c-90d8-729e5925e7d6
function diffEquationReduced(du, u, p, t)
	du[1], du[2] = fReduced(u[1], u[2], p)
end

# ╔═╡ 5e1386aa-f46f-4429-84ea-4478b4ea1e39
function integrateSystemReduced(p, u0, times)
	...
	sol = solve(prob, ROCK2(), saveat = timesScaled)
...
# ╔═╡ 9ce17343-6707-462b-be49-efe214f4e202
baseP = (
	g = 10.0,
	r = 0.2,
	T_star = 100.0,
...
# ╔═╡ ddb6bb1d-7551-4add-8467-cc68e8080140
...
# ╔═╡ 90cbe6fd-71a3-4796-8a00-8f58a58e9da9
...
# ╔═╡ 4d5345f7-8d3d-489d-8895-ebd11d705e32
...
# ╔═╡ 1b3ac257-f319-4041-b953-1166d9940cb1
function makeStreamPlotReduced(fig, params, (minX, maxX, minY, maxY); show_bassin=false)
    ...
    if show_bassin == true
        ...
        heatmap!(rightPannelAx1, xx, yy, mat, colormap = [:grey40], alpha=0.2)
    end
	...
	for i in getZeros(params, lims[1:2], lims[3:4])
		...
		if (sign(real(eigenValues[1])) < 0) && (sign(real(eigenValues[2])) < 0)	# attractor
			scatter!(rightPannelAx1, i[1], i[2], color = :black, markersize = 15)
		elseif (sign(real(eigenValues[1])) > 0) && (sign(real(eigenValues[2])) > 0)	# repelor
			...
			scatter!(rightPannelAx1, i[1], i[2], color = :black, markersize = 20, marker = :cross)
		end
...
# ╔═╡ 49a58277-3b22-4444-8485-468c573c2f29
let fig = Figure(size = (900, 700))

	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = baseP
	
	makeStreamPlotReduced(fig[1, 1], [g, 0.1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], (-3, 8, -2, 4), show_bassin = true)
	makeStreamPlotReduced(fig[1, 2], [g, 2, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], (-3, 8, -2, 4), show_bassin = true)
	makeStreamPlotReduced(fig[1, 3], [g, 20, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], (-3, 8, -2, 4), show_bassin = true)


	#img = load(assetpath(string(pwd(), "/inkscape/fig_lotka_volterra.png")))

	#ax_A = Axis(fig[0, :], aspect = DataAspect(), yreversed = true,)
	#image!(ax_A, img')

	#hidedecorations!(ax_A)
	#hidespines!(ax_A)

	#rowsize!(fig.layout, 0, 350)

	label_size = 30
	pad = (0, 5, 6, 0)
	off_set = (0, 0)
	# middle plot 
	Label(fig[0, :, TopLeft()], "A)", fontsize = label_size, font = :bold, padding = pad, halign = :right)
	Label(fig[1, 1, TopLeft()], "B)", fontsize = label_size, font = :bold, padding = pad, halign = :right)
	Label(fig[1, 2, TopLeft()], "C)", fontsize = label_size, font = :bold, padding = pad, halign = :right)
	Label(fig[1, 3, TopLeft()], "D)", fontsize = label_size, font = :bold, padding = pad, halign = :right)
	rowgap!(fig.layout, 4)

	save("makieExports/figS10.png", fig)
	fig
...
manifest_format = "2.0"
project_hash = "023301c42fa957a929ea0382dacac5c9041a6a7a"
