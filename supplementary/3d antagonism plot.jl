...
using DifferentialEquations, CairoMakie, Roots, ForwardDiff, LinearAlgebra, PlutoUI, FileIO, Colors 
...
function outcomeMatrix2Species(p, u0)
	...
	g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = p
	_, u2, u3 = u0
	I1s = 10 .^ LinRange(-1, 3., 21)
	u1s = 10 .^ LinRange(-3, 6., 21)

	mat = fill(-1, size(u1s)..., size(I1s)...)
...
# ╔═╡ 4b5cbe4a-ca06-431f-aea7-1e5b88066884
...
function outcomeMatrix(p, u0)
	...
	u1, u2, u3 = u0
	I2s = 10 .^ LinRange(-1, 3., 21)
	r2s = 10 .^ LinRange(-1, 1.1, 21)
...
function plotRuns(p, u0, fig; showLegend = false, showI1 = true, showI2 = true, title = "")
	...
	ax2 = Axis(fig[1, 1], yticklabelcolor = T_color, ylabel = "T-Cells", yscale = log10, xgridvisible = false, ygridvisible = false)
...
function plotTrajectories(p, u0, fig; showLegend = false, showI1 = true, showI2 = true, title = "")
	...
	if showI1 
		...
		lines!(ax1, x, y; color=:orange,  linewidth=4, linestyle = :dash)
...
# ╔═╡ 5fbd1219-463e-412c-b990-d4faa37fd5f2
function makeContourFig(contourfig)

	g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = baseP
	u1, u2, u3 = baseU0
	
	ax6 = Axis(contourfig[2, 1], 
		xlabel = "T-Cells",
		ylabel = "Infected Cells",
	...
	Legend(contourfig[1, 1], tellwidth = false, orientation = :horizontal, nbanks = 2,
	    	[
...
# ╔═╡ be16a747-a7db-479d-afd5-0d31fa9b710b
function makeContourFig3D(contourfig)

	g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = baseP
	u1, u2, u3 = baseU0
	
	ax6 = Axis3(contourfig[1, 1], 
		xlabel = "T",
		ylabel = "I₁",
		zlabel = "I₂",
		#xscale = log10, yscale = log10,
	...
	#colors = [:lightskyblue1, :deepskyblue1, :dodgerblue1, :dodgerblue4]
	#colors = [palette(:PuBu_5)[i] for i in 1:5]
	colors = [:black, :grey40, :grey60, :grey80, :grey90]
	#vals = [0.5, 50, 100, 300, 330]
	vals = LinRange(0.5, 330, 8)
	colors = range(colorant"mediumblue", stop=colorant"orange", length=length(vals))
	...
	for (I2, color) in zip(vals, colors)
		...
		contourf!(ax6, xx, yy, mat*I2, levels = [0, I2], colormap = [color], #labels = true
		)
	...
	# compute solution
	sol = integrateScaled(
			[u1, u2, 100], [g, 2.5, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 0.2], LinRange(0, 50, 2000))
	...
	I2_color = :tomato1
	alphaAmount = 0.7
	linecolor = range(colorant"black", stop=colorant"red", length=length(I2_traj))
	lines!(ax6, log10.(T_traj), log10.(I1_traj), (I2_traj), linewidth = 4, color = linecolor, alpha = 1.0)
	...
	for (I2, color) in zip(vals, colors)
		...
		contour3d!(ax6, (xx), (yy), mat*I2,
			 linewidth = 4, color = color, alpha = 0.3, levels = [0.9999, 1]*I2,
		)
	end
...
# ╔═╡ 1253febb-bbe9-48eb-a778-b515f7f951f1
let fig = Figure()
	makeContourFig3D(fig[1, 1])
...
function makePhaseDiagram1(fig, low_r1, r2_cross_section)
	...
	xx, yy, mat = outcomeMatrix([g, low_r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2], [u1, 0.0001, 10])
...
# ╔═╡ 51a4db3c-9805-4059-80b1-9029487f39e8
...
let 
	...
	img = load(assetpath(string(pwd(), "/inkscape/fig_multispecies_concept_3.png")))
	...
	hidespines!(ax_A)
	rowsize!(fig.layout, 1, 475)
	#rowsize!(fig.layout, 1, Aspect(1, 1))



	# Example of combination of runs
	combinationFig = fig[2, 1] = GridLayout()
	...
	contourfig = fig[2, 2] = GridLayout()
	makeContourFig(contourfig)

	# Phase diagram 1
	phaseFig = fig[3, :] = GridLayout()
	...
	xoff = 10
	yoff = -label_size
	text!(fig.scene, labV1, 1, text = "A)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	text!(fig.scene, 0.5, 1, text = "B)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	text!(fig.scene, labV1, 0.8, text = "C)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	text!(fig.scene, labV1, labH2, text = "D)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	text!(fig.scene, labV2, labH2, text = "E)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	text!(fig.scene, labV1, labH3, text = "F)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	text!(fig.scene, labV2, labH3, text = "G)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	...
	save("exports/fig4_20241216.png", fig)
...
manifest_format = "2.0"
project_hash = "1cb42cbf566090e40f4ce19afb8e2b7c0ffb8c64"
