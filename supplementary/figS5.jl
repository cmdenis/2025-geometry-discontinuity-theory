...
# ╔═╡ 8fcf96f2-2de7-4fdb-8370-5b032a45bcb9
using DifferentialEquations, CairoMakie, FileIO, IntervalRootFinding, LinearAlgebra, ForwardDiff, PlutoUI, Interpolations

# ╔═╡ e2e423cc-73b0-4373-870d-f4c6f3e231c8
function Treg(T_star_reg, beta, delta2, mu, T, alpha)
	# Computing the T_reg equilibrium approximation
	top = -delta2*(alpha*T - beta*T_star_reg) + sqrt((T * alpha * delta2 + T_star_reg*beta*delta2)^2 + 4*T*(beta^2)*delta2*mu)
	bot = 2*delta2*beta
	return top/bot
end

# ╔═╡ a605af05-ff8d-4ec1-b50c-6e1d239c16c0
## Defining stuff for the quasistatic version:
function il2(T_star_reg, beta, delta2, mu, T, alpha)
    return (Treg(T_star_reg, beta, delta2, mu, T, alpha) - T_star_reg)*delta2/beta
end

# ╔═╡ dd4e6409-02ae-44a1-811c-b5b02228cb41
...
# ╔═╡ 9fe3565c-e99d-457f-b58a-ff81409af06d
function RELU(x)
	return max(0, x) 
end

# ╔═╡ 055ad66d-32dd-4974-80a5-519b8c07740e
function diffEquationQuasistatic(x, y, p)
	# system of equation for the quasistatic system
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = p
    
	du1 = kappa*y*(T_star + alpha*x*il2(T_star_reg, beta, delta2, mu, RELU(x), alpha)/Treg(T_star_reg, beta, delta2, mu, RELU(x), alpha)/delta1) - gamma*x

	n = 10
	k = 1
	# Differential equations
	du2 = (g + r*y - kappa*y*(x + T_star))*crossfader(y, n, k) - y*(1-crossfader(y, n, k)) 
	return du1, du2
end

# ╔═╡ 3eb4873f-74a6-413d-8bf5-369126797b24
function diffEquationQuasistaticNoRELU(x, y, p)
	# system of equation for the quasistatic system
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = p
    
	du1 = kappa*y*(T_star + alpha*x*il2(T_star_reg, beta, delta2, mu, (x), alpha)/Treg(T_star_reg, beta, delta2, mu, (x), alpha)/delta1) - gamma*x
...
# ╔═╡ 5da654af-0352-427f-b2a3-97bb643d9ba0
function fQuasistatic(du, u, p, t)
	du[1], du[2] = diffEquationQuasistatic(u[1], u[2], p)
end

# ╔═╡ ea637b81-29f4-4605-8a20-4054a9d46ef8
function integrateSystemQuasistatic(p, u0, times)
	prob = ODEProblem(fQuasistatic, u0, (times[1], times[end]), p);
	sol = solve(prob, Rodas4P(), saveat = times[2]-times[1], abstol=1e-9, reltol=1e-6);
	return sol
end

# ╔═╡ edf46e2c-c0ff-4a12-84cc-dc0286356432
function isacute(p, u0; threshhold = 0.05)
    tt = 0:0.01:500.0

    # Integrate the system
    sol = integrateSystemQuasistatic(p, u0, tt);
...
# ╔═╡ 9d639587-d640-40c5-b05c-9facf10de966
function makeBassins(xlimits, ylimits, p)
    ...
    for (i, xx) in enumerate(xspace)
        ...
        for (j, yy) in enumerate(yspace)
			...
			#println(isacute(p, [10^xx, 10^yy], f))
            if isacute(p, [10^xx, 10^yy]) == false
				mat[i, j] = 1
			end
        end
    end
...
# ╔═╡ 3db486e7-0267-45f0-885d-f4eca7c04f8c
function logSpaceDiffEquation(x, y, p)
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = p
	scaled_x = (10^x)
	scaled_y = (10^y)
    dx, dy = diffEquationQuasistatic(scaled_x, scaled_y, p)
    return [dx / (scaled_x), dy / scaled_y]
end

# ╔═╡ 804db3d3-e552-470b-9011-896d66033e51
function logSpaceDiffEquationNoRELU(x, y, p)
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = p
	scaled_x = (10^x)
	scaled_y = (10^y)
    dx, dy = diffEquationQuasistaticNoRELU(scaled_x, scaled_y, p)
    return [dx / (scaled_x), dy / scaled_y]
end

# ╔═╡ b02492be-5a33-4121-9dff-8f44b95308cc
function fQuasistaticLinearized(x, y, p)
	# outputs the jacobian at a given point
	return ForwardDiff.jacobian(v -> [diffEquationQuasistatic(v..., p)...], [x, y])
end

# ╔═╡ 7c5e8309-4dee-475d-9450-7a6c0a869b18
function getZeros(p, xlims, ylims)
	# Get zeros of system
	rts = roots(x -> logSpaceDiffEquationNoRELU(x[1], x[2], p), [interval(xlims...), interval(ylims...)])
...
# ╔═╡ dd77c11c-c28d-45e6-9a7d-77efcd29ae2a
baseP = (
	g = 10.0,
	r = 0.2,
	T_star = 100.0,
...
# ╔═╡ 44924827-de9e-4cf4-85ca-edc04d4ba555
baseU0 = (
	T = 0.0,
	I = 1.0
)

# ╔═╡ 41ef3153-0848-4ddd-92e9-32c742a29046
function makeStreamPlotQuasistatic(fig, params, (minX, maxX, minY, maxY); show_bassin=false, final_time_chop = 20000)
    # function to make streamplot on fig
	
	# Parameters
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = params
    ...
    contour!(ax, xs, ys, FX; levels=[0.0], color=:black, linestyle=:dash, linewidth = 4)
    contour!(ax, xs, ys, FY; levels=[0.0], color=:black, linestyle=:dash, linewidth = 4)


    if show_bassin == true
        # compute bassin of attraction
        xx, yy, mat = makeBassins((minX, maxX), (minY, maxY), params)
        heatmap!(ax, xx, yy, mat, colormap = [:grey40], alpha=0.2)
    end

    streamplot!(
        ax,
        (x, y) -> Point2f(logSpaceDiffEquation(x, y, params)),
        lims[1:2],
        lims[3:4],
        density=0.5,
        colormap=[:grey70],
	...
	for i in getZeros(params, lims[1:2], lims[3:4])

		eigenValues = eigvals(fQuasistaticLinearized(10^i[1], 10^i[2], params))
		eigenVectors = eigvecs(fQuasistaticLinearized(10^i[1], 10^i[2], params))


		if (sign(real(eigenValues[1])) < 0) && (sign(real(eigenValues[2])) < 0)	# attractor
			scatter!(ax, i[1], i[2], color = :black, markersize = 15)
		elseif (sign(real(eigenValues[1])) > 0) && (sign(real(eigenValues[2])) > 0)	# repelor
			scatter!(ax, i[1], i[2], color = :white, markersize = 15)
		else
			scatter!(ax, i[1], i[2], color = :black, markersize = 20, marker = :cross)
		end
	end


	sol = integrateSystemQuasistatic(params, [10^4, 10^2], -(0:0.001:2000))
	Ttraj = log10.(sol[1, end-final_time_chop:end])
	Itraj = log10.(sol[2, end-final_time_chop:end])
	lines!(ax, Ttraj, Itraj, color = :blueviolet, linewidth = 4, linestyle = :dot)
    ...
    #obj = lines!(ax, x, y; color=c, colormap=Reverse(:managua100), linewidth=4)
    #Colorbar(fig[1, 2], obj, label="Time (days)")
end

# ╔═╡ 608ca2a0-1fda-4828-9e1b-4751079e5641
function makeStreamPlotQuasistatic1(fig, params, (minX, maxX, minY, maxY); show_bassin=false)
    # function to make streamplot on fig
    ...
    if show_bassin == true
        # compute bassin of attraction
        xx, yy, mat = makeBassins((minX, maxX), (minY, maxY), params)
        heatmap!(ax, xx, yy, mat, colormap = [:black], alpha=0.1)
    end
...
# ╔═╡ 7b7d48a4-d844-4278-bd6a-258012ac773e
function limitCycleBoundsPlotting(pointX, pointY)
	# function to return the upper and lower boundaries of a limit cycle so that I can shade its inner area

	
	minX, minXidx = findmin(pointX)
	leftPoint = [minX, pointY[minXidx]]
	maxX, maxXidx = findmax(pointX)
	rightPoint = [maxX, pointY[maxXidx]]
	minY, minYidx = findmin(pointY)
	bottomPoint = [pointX[minYidx], minY]
	maxY, maxYidx = findmax(pointY)
	topPoint = [pointX[maxYidx], maxY]
	#println(minX, minY, maxX, maxY)

	# find first time the cycle goes to the bottom left of the limit cycle
	# and remove everything before
	idx = findfirst((pointX .< bottomPoint[1]) .& (pointY .< leftPoint[2]))
	pointX = pointX[idx:end]
	pointY = pointY[idx:end]

	

	# find first point that crosses through the next segment and chop it too
	idx = findfirst((pointX .<= topPoint[1]) .& (pointY .>= leftPoint[2]))
	pointX = pointX[idx:end]
	pointY = pointY[idx:end]

	#return [pointX[1:500], pointY[1:500]], []

	# now we are garanteed that the first point starts on the left corner of the limit cycle, so we can search for the first point that ends crosses the bottom right corner
	idx = findfirst((pointX .>= bottomPoint[1]) .& (pointY .<= rightPoint[2]))
	topX = pointX[1:idx]*1
	topY = pointY[1:idx]*1
	...
	#return topX[1:end-200], topY[1:end-200], topY[1:end-200]

	# get the lower half of the cycle
	idx = findfirst((pointX .< bottomPoint[1]) .& (pointY .>= leftPoint[2]))
	botX = pointX[1:idx]*1
	botY = pointY[1:idx]*1
	p = sortperm(botX)	# sort the list for the interpolation step
	botX = botX[p]
	botY = botY[p]

	#return botX[1:end-200], botY[1:end-200], botY[1:end-200]
	

	# need to do an interpolation between points since top and bottom curve don't have the same length, and we need them to to make the band plot...
	botInterpolation = linear_interpolation(botX, botY)
	
	xvals = topX[2:end-2]
	topvals = topY[2:end-2]
	botvals = botInterpolation.(xvals)

	return xvals, topvals, botvals
end

# ╔═╡ b321784b-5d22-48ce-afae-93f6ea166c1f
function makeStreamPlotQuasistatic2(fig, params, (minX, maxX, minY, maxY); show_bassin=false)
    # function to make streamplot on fig
	lc_color = :purple
	...
	# make limit cycle trajectory
	sol = integrateSystemQuasistatic(params, [10, 10], 0:0.004:2000)
	Ttraj = log10.(sol[1, 200000:end])
	Itraj = log10.(sol[2, 200000:end])
	xvals, top, bot = limitCycleBoundsPlotting(Ttraj, Itraj)
	#band!(ax, xvals, top, bot, color = :blueviolet, alpha = 0.5)
	lines!(ax, xvals, top, color = :blueviolet, linewidth = 4)
	lines!(ax, xvals, bot, color = :blueviolet, linewidth = 4)

	sol = integrateSystemQuasistatic(params, [10^3.5, 10^1.8], -(0:0.001:2000))
	Ttraj = log10.(sol[1, 40000:end])
	Itraj = log10.(sol[2, 40000:end])
	xvals, top, bot = limitCycleBoundsPlotting(reverse(Ttraj), reverse(Itraj))
	#band!(ax, xvals, top, bot, color = :blueviolet, alpha = 0.5)
	lines!(ax, xvals, top, color = :blueviolet, linewidth = 4, linestyle = :dot)
	lines!(ax, xvals, bot, color = :blueviolet, linewidth = 4, linestyle = :dot)
...
# ╔═╡ c52f3e64-5c39-406d-a5b6-9c7ae9606cde
function showRuns(p, u0, fig)

	# times for simulation
	times = LinRange(0, 50, 2000)


	# compute solution
	solQuasistatic = integrateSystemQuasistatic(p, u0, -times)

	# removing values that are too low from trajectories (for plotting scale)
	T_trajQuasistatic = solQuasistatic[1, :]
	T_trajQuasistatic[T_trajQuasistatic .< 1e-3] .= NaN
	
	I_trajQuasistatic = solQuasistatic[2, :]
	I_trajQuasistatic[I_trajQuasistatic .< 1e-3] .= NaN
	...
	ax2 = Axis(fig[1, 1], yticklabelcolor = I_color, ylabel = "Infected Cells", yscale = log10, ygridvisible = false)
	xlims!(ax1, 0, times[end])
	xlims!(ax2, 0, times[end])
	
	lines!(ax1, times, T_trajQuasistatic, color = T_color, linewidth = 4)
	lines!(ax2, times, I_trajQuasistatic, color = I_color, linewidth = 4)
	...
	Legend(fig[1, 1],
    	[
			[LineElement(color = I_color, points = Point2f[(0, 0.75), (1, 0.75)], linewidth = 4), LineElement(color = T_color, points = Point2f[(0, 0.25), (1, 0.25)], linewidth = 4)],
			[LineElement(color = I_color, points = Point2f[(0, 0.75), (1, 0.75)], linewidth = 4, linestyle = (:dot,:dense)), LineElement(color = T_color, points = Point2f[(0, 0.25), (1, 0.25)], linewidth = 4, linestyle = (:dot,:dense))]
		],
    	["Quasistatic", "Reduced"], tellwidth = false, tellheight = false, orientation = :horizontal, height = 23, halign = :right, valign = :bottom
	)
...
# ╔═╡ 07120f80-f97b-494b-871b-adf0a9d4d7fc
function tracesFixedPoints(rs, params, lims; maxFP = 2)

	g, _, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = params

	# first get all the points for the range in rs
	FP = fill(-10.0, length(rs), maxFP, 2)
	
	for (i, r) in enumerate(rs)
		zers = hcat(getZeros([g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], lims[1:2], lims[3:4])...)'
		#println(zers)
		#println(size(zers))
		#println(FP[i, 1:size(zers)[2], :])
		
		FP[i, 1:size(zers)[2], :] = zers
	end


	# rearrange the points, so that by continuity the same points follow each other in the sam row/col
	for (i,r)  in enumerate(rs[1:end-1])
		for j in 1:maxFP
			FPtemp = FP[i + 1, :, :]*1
	
			# find closest point to the previous one
			idx = findmin(
				sqrt.((FP[i, j, 1] .- FPtemp[:, 1]).^2 + (FP[i, j, 2] .- FPtemp[:, 2]).^2)
			)[2]
...
# ╔═╡ c7479fc4-a6af-480c-be84-f48743a56795
function getBassinBounds(rs, params, u0)
	# function to get the bounds of the bassin of attraction/limit cycle

	# parameters for rescaling
	g, _, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = baseP

	# rescale time and initial conditions
	times = -(0:0.001:200)
	postimes = 0:0.01:5000
	#println((times[1], times[end]))
	...
	for r in rs
		prob = ODEProblem(
			...
			u0, 
			(times[1], times[end]), 
			[g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu]);
		...
		# Store values
		push!(Tmax_unstable, maximum(sol[1, end-10000:end]))
		push!(Imax_unstable, maximum(sol[2, end-10000:end]))
		push!(Tmin_unstable, minimum(sol[1, end-10000:end]))
		push!(Imin_unstable, minimum(sol[2, end-10000:end]))

		# go outside limit cycle to try and catch the stable one
		idx = findmax(sol[1, end-10000:end])[2]
		newU0 = [Tmax_unstable[end]+100, (sol[2, end-10000:end])[idx]]
		prob = ODEProblem(
			fQuasistatic, 
			newU0, 
			(postimes[1], postimes[end]), 
			[g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu]);
		
		sol = solve(prob, AutoTsit5(Rodas4P()), saveat = postimes, abstol=1e-9, reltol=1e-6)
		ll = length(sol[1, :])
		if (maximum(sol[1, end-min(ll, 10000)+1:end]) - minimum(sol[1, end-min(ll, 10000)+1:end])) > 1000
			push!(Tmax_stable, maximum(sol[1, end-min(ll, 10000)+1:end]))
			push!(Imax_stable, maximum(sol[2, end-min(ll, 10000)+1:end]))
			push!(Tmin_stable, minimum(sol[1, end-min(ll, 10000)+1:end]))
			push!(Imin_stable, minimum(sol[2, end-min(ll, 10000)+1:end]))	
		else
			push!(Tmax_stable, NaN)
			push!(Imax_stable, NaN)
			push!(Tmin_stable, NaN)
			push!(Imin_stable, NaN)	
		end
		
	end
	#println(Imin)

	Tmax_unstable[Imin_unstable .< 1e-3] .= NaN
	Imax_unstable[Imin_unstable .< 1e-3] .= NaN
	Tmin_unstable[Imin_unstable .< 1e-3] .= NaN
	Imin_unstable[Imin_unstable .< 1e-3] .= NaN

	Tmax_stable[Imin_stable .< 1e-3] .= NaN
	Imax_stable[Imin_stable .< 1e-3] .= NaN
	Tmin_stable[Imin_stable .< 1e-3] .= NaN
	Imin_stable[Imin_stable .< 1e-3] .= NaN

	return [log10.(Tmax_unstable), log10.(Imax_unstable), log10.(Tmin_unstable), log10.(Imin_unstable)], [log10.(Tmax_stable), log10.(Imax_stable), log10.(Tmin_stable), log10.(Imin_stable)]


end

# ╔═╡ 88065a4b-e29a-421d-826c-4ba4283b625b
function makeBifurcationDiagram(fig, rvals, names)
	
	rs = 10 .^ LinRange(-2, 1, 200)


	ax = Axis(fig[1, 1], xscale = log10, xlabel = "Growth rate (r)", ylabel = "T-Cells (log10)")



	
	# draw limit cycle
	bassinUnstable, bassinStable = getBassinBounds(rs, baseP, [10^4, 10^1.7])

	# solid region
	band!(ax, rs, bassinUnstable[1], bassinUnstable[3], alpha = 0.5, color = :gray40)
	band!(ax, rs, bassinStable[1], bassinStable[3], alpha = 0.3, color = :gray40)
	# boundaries
	lines!(ax, rs, bassinUnstable[1], color = :blueviolet, linewidth = 4, linestyle = :dot)
	lines!(ax, rs, bassinUnstable[3], color = :blueviolet, linewidth = 4, linestyle = :dot)
	lines!(ax, rs, bassinStable[1], color = :blueviolet, linewidth = 4)
	lines!(ax, rs, bassinStable[3], color = :blueviolet, linewidth = 4)

	# draw fixed points
	rs = 10 .^ LinRange(-2, 1, 10)
	fp = tracesFixedPoints(
		rs, 
		baseP, 
		(-3, 6, -1, 4)
	)
	lines!(ax, rs, fp[:, 1, 1], linewidth = 4, color = :black, linestyle = :dash)
	lines!(ax, rs, fp[:, 2, 1], linewidth = 4, color = :black)

	


	# vertical lines
	vlines!(ax, rvals, color = :black)
	text!(ax, rvals, ones(length(rvals))*2, text = names, offset = (10, 0), fontsize = 20, font = :bold)
	
end

# ╔═╡ a4f682dc-2c22-458c-bda9-4dc803e380fa
let 
	h1 = 0.98
	h2 = 0.66
	h3 = 0.33
	...
	# parameters for rescaling
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = baseP

	rvals = [0.03, 0.278, 0.9, 1.92434, 5]

	figwidth = 220

	makeStreamPlotQuasistatic2(fig[1, 1], [g, rvals[1], T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], (-4, 6, -1, 3.1), show_bassin=true)
	makeStreamPlotQuasistatic2(fig[1, 2], [g, rvals[2], T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], (-4, 6, -1, 3.1), show_bassin=true)
	makeStreamPlotQuasistatic1(fig[1, 3], [g, rvals[3], T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], (-4, 6, -1, 3.1), show_bassin=true)
	
	row2 = fig[2, :] = GridLayout()
	makeStreamPlotQuasistatic(row2[1, 1], [g, rvals[4], T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], (-4, 6, -1, 3.1), show_bassin=true, final_time_chop=6000)
	makeStreamPlotQuasistatic(row2[1, 2], [g, rvals[5], T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], (-4, 6, -1, 3.1), show_bassin=true, final_time_chop = 2560)
	
	Legend(row2[1, 3], tellwidth = true, orientation = :vertical,
	    	[
				[
					LineElement(color = :blueviolet, linewidth = 4, alpha = 1.0),
					LineElement(color = :blueviolet, linewidth = 4, alpha = 1.0, linestyle = :dot),
					MarkerElement(color = :black, markersize = 15, marker = :dot),
					MarkerElement(color = :black, markersize = 20, marker = :cross),
					MarkerElement(color = :grey60, markersize = 20, marker = :rect),
	...
	[colsize!(fig.layout, i, figwidth) for i in 1:3]
	colsize!(row2, 1, figwidth)
	colsize!(row2, 2, figwidth)
	...
	xoff = 10
	yoff = -label_size
	text!(fig.scene, v1, h1, text = "A)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	text!(fig.scene, v3, h1, text = "B)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	text!(fig.scene, v5, h1, text = "C)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	text!(fig.scene, v2, h2, text = "D)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	text!(fig.scene, v4, h2, text = "E)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))
	text!(fig.scene, v1, h3, text = "F)", fontsize = label_size, font = :bold, space = :relative, offset = (xoff, yoff))

	save("makieExports/figS5.png", fig)
	
	fig
end

# ╔═╡ 9b26e234-ec39-448c-8a10-ecc341bf52e1
# ╠═╡ disabled = true
#=╠═╡
let
	fig = Figure(size = (900, 800))
	...
	makeStreamPlotQuasistatic2(fig[1, 1], [g, rvals[1], T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], (-4, 6, -1, 4), show_bassin=true)
...
manifest_format = "2.0"
project_hash = "54338da9dd43a834ee389160c72d8b5c706ad51b"
