...
# ╔═╡ 8ade5fe6-a91a-11ef-16ee-af3d8056aaa6
using DifferentialEquations, CairoMakie, FileIO, IntervalRootFinding, LinearAlgebra, ForwardDiff, MakieExtra

# ╔═╡ c9ec7768-bd26-4be1-af09-e8cb77ad7b45
function RELU(x)
	return max(0, x) 
end

# ╔═╡ 5626b39f-2993-49a5-bf5e-d08e8fe27ebd
...
# ╔═╡ dfab99a2-6d30-41a8-8a2e-f9955857850c
...
# ╔═╡ 4e949363-e46b-488d-b56c-57b5d3502bda
function fReduced(x, y, p)
	# Parameters
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = p
    c1, c2, c3 = convertParam(p)
...
# ╔═╡ 66e2fa97-2f17-4f3f-b90d-b69a82ef645a
function diffEquationReduced(du, u, p, t)
	du[1], du[2] = fReduced(u[1], u[2], p)
end

# ╔═╡ 283684e3-01f9-4226-ad81-f196dfa190b1
function integrateSystemReduced(p, u0, times)
	# parameters for rescaling
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = p


	# rescale time and initial conditions
	timesScaled = times .* gamma
	u0Scaled = [u0[1]*kappa/gamma, u0[2]*gamma/g]

	prob = ODEProblem(diffEquationReduced, u0Scaled, (timesScaled[1], timesScaled[end]), p);
	sol = solve(prob, Rodas4P(), saveat = timesScaled, abstol=1e-9, reltol=1e-6)

	# scaling the variables
	Tcells = sol[1, :]*gamma/kappa
	Icells = sol[2, :]*g/gamma
	
	return hcat(Tcells, Icells)'
end

# ╔═╡ b047c90f-c380-444d-b883-726dbf0c928a
function acuteReduced(p, u0)
	# function to check if the system is in an acute state or a chronic state
	times = LinRange(0., 1000, 4000)
	sol = integrateSystemReduced(p, u0, times)

	threshOscillations = 2

	# check if chronic
	if (sol[2, end] < 0.01)
		return 1
	else
		if getRange(sol[2, end-100:end]) > threshOscillations
			return 3
		else
			return 2
		end
...
# ╔═╡ 6aa9fd0c-239c-497e-96ef-8b5672589038
function makePlotReduced(p, IC, fig)
	# function to plot the result of infections
	...
	rs = LinRange(0, 3, 80)
	u02s = LinRange(0, 500, 80)

	# matrix containing outcomes
	mat = fill(NaN, length(rs), length(u02s))
	
	for (i, r) in enumerate(rs)
		for (j, u02) in enumerate(u02s)
			mat[i, j] = acuteReduced(
	...
	ax1, hm = heatmap(
		fig[1, 1], 
		rs, u02s, mat,
		colormap = cgrad(:PuBu_3, 3; categorical = true),
		colorrange = (0.5, 3.5),
		nan_color = :white,
		axis=(;ylabel="Initial Challenge", 
				...
				ylabelsize = 20, xlabelsize = 20, xticklabelsize = 20, yticklabelsize = 20,
			  	limits = (0, 3, 0, 500)
				#title = string("Reduced System"
					#"g=", g, "\tT*=", T_star, "\tTreg*=",T_star_reg, "\tα=", alpha, "\nβ=", beta, "\tγ=", gamma, "\tδ1=", delta1, "\tδ2=", delta2, "\nκ=", kappa, "\tμ=", mu
				#)
			)
	)
	...
	# parameters to iterate over
	rs = LinRange(2.1, 2.9, 80)
	u02s = LinRange(0, 50, 80)
	...
	#text!(ax1, 0.2, 7, text = "C", fontsize = 20, align = (:center, :top), font = :bold)
	#text!(ax1, 2.0, 7, text = "F", fontsize = 20, align = (:center, :top))
	#text!(ax1, 6.0, 7, text = "D", fontsize = 20, align = (:center, :top), font = :bold)
	...
	cb = Colorbar(
		...
		ticks = (1:3, ["Acute", "Chronic", "Cyclic"]),
		#vertical = false,
		ticklabelfont = :bold,
		ticklabelsize = 20,
...
# ╔═╡ 00513e69-5e7d-4232-8e91-554cd9a52570
baseP = (
	g = 10.0,
	r = 0.2,
	T_star = 100.0,
...
# ╔═╡ 1c7ab0af-294a-44bc-a1fa-cc00b6dca43c
let
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = baseP
	

	convertParam([g, 4.0, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu])
end

# ╔═╡ 78e591ac-4d9a-4b89-a665-5cae1fd18801
baseU0 = (
	T = 0.0,
	I = 1.0
)

# ╔═╡ 0497ee28-8f93-4ec6-9f1b-109467d16033
md"## Geometrical plots

Here I focus on making the 'geometrical' plots of the paper with the phase portrait and such."

# ╔═╡ 9d683f02-ef54-4b4a-87e5-5ab45d493b5e
function fReducedLinearized(x, y, p)
	# outputs the jacobian at a given point
	return ForwardDiff.jacobian(v -> [fReduced(v..., p)...], [x, y])
end

# ╔═╡ 28b0d3b8-215d-4246-801a-aae78226b39d
function phasePortraitLinear(x, y, p)
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = p
	scaled_x = (x)*(kappa/gamma)
	scaled_y = (y)*(gamma/g)
    dx, dy = fReduced(scaled_x, scaled_y, p)
    return [dx , dy ]
end

# ╔═╡ a91d609e-1a39-452a-9b95-36cceeab6510
...
# ╔═╡ a3bb2a75-f03d-48c8-9db6-ba3d660a4b44
function getManifold(u0, params; xlims = (-3, 8), ylims = (-1, 8))
	# function to compute the manifold given a fixed point


	# parameters for rescaling
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = params
	
	eigenValues = eigvals(fReducedLinearized(10^u0[1], 10^u0[2], params))
	eigenVectors = eigvecs(fReducedLinearized(10^u0[1], 10^u0[2], params))

	trajectories = []

	epsilon = 1e-8

	# compute new point along directions of manifolds
	for (eigenVector, eigenValue) in zip([eigenVectors[:, 1], eigenVectors[:, 2]], [eigenValues[1], eigenValues[2]])
		# computing trajectories from both sides of the fixed point
		u1p = 10 .^(u0 + eigenVector*real(eigenValue)*epsilon)
		u2p = 10 .^(u0 - eigenVector*real(eigenValue)*epsilon )

		for IC in [u1p, u2p]
			println("Starting points (linear space):", IC)
			println("Sign of eigenValue: ", sign(eigenValue))
			
			# rescale time and initial conditions
			times = (0.0:0.1:500)*sign(eigenValue)
			timesScaled = times .* gamma
			u0Scaled = [IC[1]*kappa/gamma, IC[2]*gamma/g]
			println("Starting points (linear space):", u0Scaled)

			# define callback for blowups
			condition(u, t, integrator) = (u[2] > 10.0^ylims[2]) || (u[1] > 10.0^xlims[2]) || (u[2] < 10.0^ylims[1])
			cb = DiscreteCallback(condition, terminate!)
		
			prob = ODEProblem(diffEquationReduced, u0Scaled, (timesScaled[1], timesScaled[end]), params);
			#sol = solve(prob, AutoTsit5(Rodas5P()), abstol=1e-9, reltol=1e-6, callback = cb)
			sol = solve(prob, Euler(), dt = 0.0001, callback = cb)
			#sol = eulerIntegrator(
				#(x, y) -> fReduced(x, y, params),
				#u0Scaled,
				#times,
				#10.0 .^xlims,
				#10.0 .^ylims
			...
			#println("Start I: ", Icells[1])
			#println("End I: ", Icells[end])

			# storing values in list
			push!(trajectories, [Tcells, Icells])
			#println([Tcells, Icells])
...
# ╔═╡ fa2fb5de-e3a5-41e5-a319-4ac3331c0919
...
# ╔═╡ 9fd85b53-a87d-4187-be7a-4b9091030ded
function makeBassins(xlimits, ylimits, p)
    ...
    for (i, xx) in enumerate(xspace)
        ...
        for (j, yy) in enumerate(yspace)
			if isacute(p, [10^xx, 10^yy], diffEquationReduced) == false
                mat[i, j] = 1
            end
...
# ╔═╡ 3eab1637-c43b-4d11-be7a-39e42c1e8d78
function makeBassinsLinear(xlimits, ylimits, p)
    # function to create the bassin of attractions 
    ...
    for (i, xx) in enumerate(xspace)
        #println("x:", xx)
        for (j, yy) in enumerate(yspace)
			if isacute(p, [xx, yy], diffEquationReduced) == false
                mat[i, j] = 1
            end
...
# ╔═╡ caa260c4-9f55-48a7-b72c-f70be1083076
function getBassinBounds(rs, params, u0)
	...
	for r in rs
		prob = ODEProblem(diffEquationReduced, u0Scaled, (timesScaled[1], timesScaled[end]), [g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu]);
		sol = solve(prob, AutoTsit5(Rodas4P()), saveat = timesScaled, abstol=1e-9, reltol=1e-6)

		# Scale back
		push!(Tmax, maximum(sol[1, :])*gamma/kappa)
		push!(Imax, maximum(sol[2, :])*g/gamma)
		push!(Tmin, minimum(sol[1, :])*gamma/kappa)
		push!(Imin, minimum(sol[2, :])*g/gamma)
	...
	return log10.(Tmax), log10.(Imax), log10.(Tmin), log10.(Imin)


end

# ╔═╡ bdaac712-a8f6-4804-ae33-952a0b9218a3
function showRuns(p, u0, fig)

	# times for simulation
	times = LinRange(0, 20, 2000)


	# compute solution
	solReduced = integrateSystemReduced(p, u0, times)

	# removing values that are too low from trajectories (for plotting scale)
	T_trajReduced = solReduced[1, :]
	T_trajReduced[T_trajReduced .< 1e-3] .= NaN
	
	I_trajReduced = solReduced[2, :]
	I_trajReduced[I_trajReduced .< 1e-3] .= NaN
	...
	# plot result
	ax2 = Axis(fig[1, 1], yticklabelcolor = T_color, ylabel = "T-Cells", yscale = log10, ygridvisible = false, ylabelsize = 20, xlabelsize = 20, xticklabelsize = 20, yticklabelsize = 20, ylabelcolor = T_color)
	ax1 = Axis(
		fig[1, 1], 
		yticklabelcolor = I_color, 
		ylabelcolor = I_color,
	...
	ylims!(ax1, 0.01, 2000)
	ylims!(ax2, 0.001, 4*10^6)
	lines!(ax2, times, T_trajReduced, color = T_color, linewidth = 4)
	lines!(ax1, times, I_trajReduced, color = I_color, linewidth = 4)
	...
	#ax2.alignmode = Mixed(right = 0)
	#ax1.alignmode = Mixed(right = 58)
...
# ╔═╡ bcd3d348-f64f-4022-8084-1829cf85e4e1
function getZeros(p, xlims, ylims)
	# Get zeros of system
	rts = roots(x -> logSpaceDiffEquation(x[1], x[2], p), [interval(xlims...), interval(ylims...)])

	# extract actual values
	#midpoints = hcat([mid.(root_region(rt)) for rt in rts]...)
	midpoints = [mid.(root_region(rt)) for rt in rts]	
	return midpoints
end
	

# ╔═╡ d59accd0-515b-408a-a6f7-eaf0886b8b2e
function makeStreamPlotReduced(fig, params, (minX, maxX, minY, maxY); show_bassin=false, basinColor = [:aquamarine])
    ...
    contour!(rightPannelAx1, xs, ys, FX; levels=[0.0], color=:black, linestyle=:dash, linewidth = 4)
    contour!(rightPannelAx1, xs, ys, FY; levels=[0.0], color=:black, linestyle=:dash, linewidth = 4)


    if show_bassin == true
        # compute bassin of attraction
        xx, yy, mat = makeBassins((minX, maxX), (minY, maxY), params)
        heatmap!(rightPannelAx1, xx, yy, mat, colormap = basinColor, alpha=0.2)
    end
	...
	for i in getZeros(params, lims[1:2], lims[3:4])
		...
		eigenValues = eigvals(fReducedLinearized(10^i[1], 10^i[2], params))
		eigenVectors = eigvecs(fReducedLinearized(10^i[1], 10^i[2], params))


		if (sign(real(eigenValues[1])) < 0) && (sign(real(eigenValues[2])) < 0)	# attractor
			scatter!(rightPannelAx1, i[1], i[2], color = :chocolate1, markersize = 15)
		elseif (sign(real(eigenValues[1])) > 0) && (sign(real(eigenValues[2])) > 0)	# repelor
			scatter!(rightPannelAx1, i[1], i[2], color = :white, markersize = 15)
		else
			scatter!(rightPannelAx1, i[1], i[2], color = :royalblue, markersize = 20, marker = :cross)
		end
	...
	tt = (0:0.01:50)
	sol = integrateSystemReduced([g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], [10^-4, 10^0], tt)
	x = log10.(abs.(sol[1, :]))
    y = log10.(abs.(sol[2, :]))
    c = LinRange(1, tt[end], length(x))

    obj = lines!(rightPannelAx1, x, y; color=c, colormap=Reverse(:managua100), linewidth=4)
    #Colorbar(fig[1, 2], obj, label="Time (days)")
end

# ╔═╡ a0e176e1-060c-4bb7-bd38-4fce01b931d9
function makeStreamPlotReduced2(fig, params, (minX, maxX, minY, maxY); show_bassin=false, basinColor = [:aquamarine], maxTime = 1000)
    # function to make streamplot on fig
	...
	# Plot limit cycle and shaded area
	
	sol = integrateSystemReduced([g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], [10^4, 10^2], -(0:0.01:2000))
	Ttraj = log10.(sol[1, end-maxTime:end])
	Itraj = log10.(sol[2, end-maxTime:end])
	#println(sol)
	
	

	#xvals, bot, top = limitCycleBoundsPlotting(reverse(sol[1, 10000:end]), reverse(sol[2, 10000:end]))
	#band!(ax, xvals, top, bot, color = :orchid, alpha = 0.5)
	lines!(rightPannelAx1, Ttraj, Itraj, color = :aquamarine1, linewidth = 4, linestyle = :dot)
	#lines!(ax, xvals, bot, color = :orchid, linewidth = 4)
    ...
    obj = lines!(rightPannelAx1, x, y; color=c, colormap=Reverse(:managua100), linewidth=4)
    #Colorbar(fig[1, 2], obj, label="Time (days)", labelsize = 20, ticklabelsize = 20)
	#println(sol)
...
# ╔═╡ 2e53c993-aae1-4f13-95c3-af9e9a435639
...
# ╔═╡ 3c8fab55-6a9a-44bf-9e08-d2c50efda340
let
	...
	#img = load(assetpath(string(pwd(), "/inkscape/fig2_network_reduced.png")))
	#ax_A = Axis(fig[1, 1:2], aspect = DataAspect(), yreversed = true,)
	#image!(ax_A, img',)
	#hidedecorations!(ax_A)
	#hidespines!(ax_A)

	makePlotReduced(baseP, baseU0, fig[1, 1])
	#ax = makePlotFader(baseP, baseU0, subFig1)
	#rowsize!(subFig1, 2, Aspect(1, 1))



	save("makieExports/figS3.png", fig)
...
manifest_format = "2.0"
project_hash = "104b01e0d3a2a0db3458df7b82a695ccdf2d0f70"
...
git-tree-sha1 = "114a31113a07629910e23e628a200cef7dd06eda"
