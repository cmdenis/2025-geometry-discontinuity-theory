...
# ╔═╡ 4eb96822-b7f1-11ef-1521-6f92654e2e76
using DifferentialEquations, CairoMakie, FileIO, IntervalRootFinding, LinearAlgebra, ForwardDiff

# ╔═╡ 8ae7fe8d-a8e6-416a-a145-36b4e5751580
function RELU(x)
	return max(0, x) 
end

# ╔═╡ 293e53ed-437b-4d38-9a40-48d7820b9b9d
...
# ╔═╡ 72e5a50b-5a92-4d9e-b004-c748938e52e2
...
# ╔═╡ 8890b2e6-7a12-494c-92bd-43c9e2578bde
function diffEquationReduced(du, u, p, t)
	du[1], du[2] = fReduced(u[1], u[2], p)
end

# ╔═╡ 1b605327-c152-4e0e-bd32-4fca24a3629a
function integrateSystemReduced(p, u0, times)
	...
	sol = solve(prob, Rodas4P(), saveat = timesScaled)
...
# ╔═╡ b32d8b2f-170d-4a2e-b842-3eea7abb6bf3
...
# ╔═╡ ad5ba50b-e69a-4ec8-9979-c127dc00db5a
...
# ╔═╡ b3b1e3ae-2ade-4474-9394-ef46ea2d0db9
...
# ╔═╡ fd4ad646-183c-4a8a-8d80-9dea191ee34d
...
# ╔═╡ 919e5117-325f-499a-af2b-85568aee4dfb
...
# ╔═╡ ff54fa1b-9b93-4e16-9c11-dcf9c372c69d
baseP = (
	g = 10.0,
	r = 0.2,
	T_star = 100.0,
...
# ╔═╡ 12be6288-4e45-4aa6-8d42-dcff67596e27
function fReducedLinearized(x, y, p)
	# outputs the jacobian at a given point
	return ForwardDiff.jacobian(v -> [fReduced(v..., p)...], [x, y])
end

# ╔═╡ 586b3222-e3b0-4650-9112-3031f44249fd
function makeStreamPlotReduced(fig, params, (minX, maxX, minY, maxY); show_bassin=false, title = "", maxTime = 500)
    ...
    if show_bassin == true
        ...
        heatmap!(rightPannelAx1, xx, yy, mat, colormap = [:aquamarine3], alpha=0.2)
    end
...
# ╔═╡ 82481bc5-db38-47e9-a6a3-b90f526246f8
...
# ╔═╡ 0fdd63e6-0cff-4fe2-a3f4-81420d52000c
...
# ╔═╡ 7230406b-6e89-453f-ab03-927b18854e2c
function bifDiagram(fig, params)
	# define parameters
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = params
	...
	ax1 = Axis(fig[1, 1], ylabel = "T-Cells (log10)", xlabel = "Growth rate (r)", 
		title = string("γ = ", gamma)
		)
	ylims!(ax1, -4, 6.4)

	
	rs = LinRange(2.4, 12, 200)
	bassin = getBassinBounds(rs, params, [10^4, 10^2])
	band!(ax1, rs, bassin[1], bassin[3], alpha = 0.5, color = :aquamarine3)
	lines!(ax1, rs, bassin[1], color = :aquamarine3, linewidth = 4, linestyle = :dash)
	lines!(ax1, rs, bassin[3], color = :aquamarine3, linewidth = 4, linestyle = :dash)


	rs = LinRange(0.0, 12, 50)
	tr = tracesFixedPoints(rs, params, (-4, 6, -4, 5))
	bassin = getBassinBounds(rs, params, [10^4, 10^2])
	lines!(ax1, rs, tr[:, 1, 1], linewidth = 4, linestyle = :dash, color = :royalblue)
	lines!(ax1, rs, tr[:, 2, 1], linewidth = 4, color = :chocolate1)
	...
	for (pos, name) in zip(xpositions, names)
		#text!(ax1, pos+xoffset, 2, text = name, fontsize = 20, font = :bold, align = (:center, :center))
	end

	xlims!(ax1, 0, 10)
	
	#vlines!(ax1, xpositions, linewidth = 1, color = :black)

	#ax1.alignmode = Mixed(left = 0)
end

# ╔═╡ 457c7b77-9d04-4be1-9b18-98e86e8e6a9a
let 
	fig = Figure(size = (900, 500))

	# parameters for rescaling
	g, r, T_star, T_star_reg, alpha, beta, _, delta1, delta2, kappa, mu = baseP

	gammas = [ 1, 10, 20]

	for (i, gamma) in enumerate(gammas)
		makeStreamPlotReduced(
			fig[1,i] , 
			[g, 10., T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu], 
			(0, 6.5, -0.5, 3.3),
			show_bassin=true,
			title = string("γ = ", gamma, ", r = 10.0"),
			maxTime = round(Int64, 500/(gamma)^(0.56))
		)
	end

	bifDiagram(fig[2, 1], [g, r, T_star, T_star_reg, alpha, beta, 1, delta1, delta2, kappa, mu])

	bifDiagram(fig[2, 2], [g, r, T_star, T_star_reg, alpha, beta, 10, delta1, delta2, kappa, mu])

	bifDiagram(fig[2, 3], [g, r, T_star, T_star_reg, alpha, beta, 20, delta1, delta2, kappa, mu])
	...
	pad = (0, 5, 6, 0)
	off_set = (0, 0)
	Label(fig[1, 1, TopLeft()], "A)", fontsize = label_size, font = :bold, padding = pad, halign = :right)
	Label(fig[1, 2, TopLeft()], "B)", fontsize = label_size, font = :bold, padding = pad, halign = :right)
	Label(fig[1, 3, TopLeft()], "C)", fontsize = label_size, font = :bold, padding = pad, halign = :right)

	rowgap!(fig.layout, 2)

	save("makieExports/figS13.png", fig)
...
manifest_format = "2.0"
project_hash = "e796f865915f82f07791eefc848fa089529c31e5"
