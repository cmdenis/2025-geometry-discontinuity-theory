...
# ╔═╡ ca214522-b8da-11ef-11de-9366903eeac8
using CairoMakie, DifferentialEquations, IntervalRootFinding, LinearAlgebra, ForwardDiff

# ╔═╡ 4c737703-c925-44c2-8ec7-0b479922cc26
function RELU(x)
	return max(0, x)
end

# ╔═╡ f53efe5b-cc07-4483-96a7-56792f3bf6cb
...
# ╔═╡ 0601b80c-b4b5-4411-97f4-563cf15839ad
...
# ╔═╡ 5549f639-7706-46dc-a4e0-f633fcbc0e4a
...
# ╔═╡ 10b08c65-85e7-4178-bd7e-13eff48a0568
function diffEquation(x, y, p)
	
	# Parameters
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, n = p
...
# ╔═╡ fe956c76-1832-4f81-9c40-f0ac21846c19
function f(du, u, p, t)
    du[1], du[2] = diffEquation(u[1], u[2], p)
end

# ╔═╡ e3ae3aec-aabe-4208-93fb-5aca6b784b81
function isacute(p, u0, system; threshhold=0.01)
	...
	prob = ODEProblem(
		...
		[u1, u2], 
		(tt[1], tt[end]), 
		[g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, n]
	)
...
# ╔═╡ 0e4b2a07-d2c5-47dc-8fce-4738e7de3c91
...
# ╔═╡ 41e3af39-931d-4f83-9fb5-96817cb43bcb
...
# ╔═╡ 2f885925-ea7c-44ce-a03f-644d7ee8a784
...
# ╔═╡ 614882ad-8c0a-4437-9f12-244a746e02c0
function makeStreamPlot(fig, params, (minX, maxX, minY, maxY); show_bassin=true)
    # function to make streamplot on fig
	
	# Parameters
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, n = params
...
# ╔═╡ 39828384-f041-45d9-ab5e-c45cd1286043
function makeCrossfadePlot(fig, n, k,)
	...
	ax2 = Axis(fig[1, 1], yticklabelcolor = col2, ylabel = L"1-H")
	...
	# make lines
	if n > 10
		ax1.title = "Hard Threshold, k = 1"
		lines!(ax1, [0, 1], [0, 0], color = col1, linewidth = 4)
		lines!(ax1, [1, 4], [1, 1], color = col1, linewidth = 4)
		lines!(ax1, [1, 1], [0, 1], color = col1, linewidth = 4)
		
		lines!(ax2, [0, 1], [1, 1], color = col2, linewidth = 4)
		lines!(ax2, [1, 4], [0, 0], color = col2, linewidth = 4)
		lines!(ax2, [1, 1], [0, 1], color = col2, linewidth = 4, linestyle = :dash)


	else
		ax1.title = string("n = ", n, ", k = ", k)
		xx = LinRange(0, 4, 100)
		yy1 = crossfader.(xx, n, k)
		yy2 = 1 .- crossfader.(xx, n, k)
		lines!(ax1, xx, yy1, linewidth = 4, color = col1)
		lines!(ax2, xx, yy2, linewidth = 4, color = col2)	
	end
	linkxaxes!(ax1, ax2)
end

# ╔═╡ 284975b8-066a-4b9b-8a62-ab240ce0b3aa
baseP = (
	g = 10.0,
	r1 = 0.2,
	T_star = 100.0,
...
# ╔═╡ f5aae173-fc98-47b1-ae26-6be359d8901e
...
# ╔═╡ 63a08991-759c-4ac8-bb1f-c6122b6eddc9
function fReducedLinearized(x, y, p)
	# outputs the jacobian at a given point
	return ForwardDiff.jacobian(v -> [diffEquation(v..., p)...], [x, y])
end

# ╔═╡ 41abe59c-ab94-43c9-8b77-4401748bf14a
let 
	g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, _ = baseP
	fig = Figure(size = (900, 500))

	for (i, n) in enumerate([2, 4, 12])
		makeCrossfadePlot(fig[1, i], n, 1)
		makeStreamPlot(
			fig[2, i],
			[g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, n],
			(-3, 7, -1, 3)
		)

		makeStreamPlot(
			fig[3, i],
			[g, 6.0, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, n],
			(-3, 7, -1, 3)
		)
	end

	labelSize = 30
	Label(fig[1, 1, TopLeft()], "A)", font = :bold, fontsize = labelSize)
	Label(fig[1, 2, TopLeft()], "B)", font = :bold, fontsize = labelSize)
	Label(fig[1, 3, TopLeft()], "C)", font = :bold, fontsize = labelSize)

	
	rowgap!(fig.layout, 5)

	save("makieExports/figS11.png", fig)
...
manifest_format = "2.0"
project_hash = "5ebb7f1bd55dec3f7b693ce53fb2da9d2055ea99"
