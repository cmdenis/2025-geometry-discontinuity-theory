...
# ╔═╡ c654b766-a760-11ef-1d12-1bd737389e5b
using DifferentialEquations, CairoMakie, PlutoUI

# ╔═╡ cfc6550c-857f-4187-a85e-0fb7b1d5fe8c
...
# ╔═╡ 7323517d-ba47-4df5-93a9-12e4fa2bfcd9
function RELU(x)
	return max(0, x)
end

# ╔═╡ 3dcc5a3a-f2a6-4774-b473-4d261d9bdb09
...
# ╔═╡ 25818892-6ca1-49ef-9d4c-123d2b57e249
...
# ╔═╡ a3b5e87e-c39c-49a4-b9d1-97fb4cce1af3
function diffEquationQuasistatic(x, y, z, p)
	# system of equation for the quasistatic system
	g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = p
    
	du1 = kappa*(y + z)*(T_star + alpha*x*il2(T_star_reg, beta, delta2, mu, RELU(x), alpha)/Treg(T_star_reg, beta, delta2, mu, RELU(x), alpha)/delta1) - gamma*x

	n = 10
	k = 1
	# Differential equations
	du2 = (g + r1*y - kappa*y*(x + T_star))*crossfader(y, n, k) - y*(1-crossfader(y, n, k)) 

	du5 = (g + r2*z - kappa*z*(x + T_star))*crossfader(z, n, k) - z*(1-crossfader(z, n, k)) 
	
	return du1, du2, 0, 0, du5
end

# ╔═╡ f3976553-7a16-443d-b4eb-84121dc87e5a
function fQuasistatic(du, u, p, t)
	du[1], du[2], du[3], du[4], du[5] = diffEquationQuasistatic(u[1], u[2], u[5], p)
end

# ╔═╡ 1790350b-eefc-4602-98a1-091d06d5e818
function integrateAntagonismFader(p, u0, times)
	prob = ODEProblem(fQuasistatic, u0, (times[1], times[end]), p);
	sol = solve(prob, Rodas4P(), saveat = times[2]-times[1], abstol=1e-9, reltol=1e-6);
	return sol
end

# ╔═╡ 6d22c228-7d99-41a1-953c-b40d2602e912
md"Max time: $@bind maxTime NumberField(0:200., default=50.)"

# ╔═╡ 030d5d48-fed8-46de-8f8d-6ca83810bfd6
times = 0:0.01:maxTime

# ╔═╡ 83eed8e4-93f9-4759-9d9b-cd40ebb9301b
md"log₁₀(T(0)): $(@bind u01 PlutoUI.Slider(-10:0.001:4, -10., true)) || log₁₀(I₁(0)): $(@bind u02 PlutoUI.Slider(-1.0:0.01:4.0, 10.0, true))"

# ╔═╡ af20fc9f-1818-4b34-8d7e-ecbed9756cc8
md"log₁₀(I₂(0)): $(@bind u05 PlutoUI.Slider(-1.0:0.01:4.0, 10.0, true))"

# ╔═╡ a39cf6d3-9a30-4471-80cf-a4d928fe4600
md"log₁₀(Treg(0)): $(@bind u03 PlutoUI.Slider(-4:0.001:4, 4., true)) || log₁₀(IL2(0)): $(@bind u04 PlutoUI.Slider(-4:0.001:4, -4., true))"

# ╔═╡ 432e1475-fef7-4b59-8d40-5f2b674ec329
u0base = 10 .^[u01, u02, u03, u04, u05]

# ╔═╡ be76b0a8-dd54-419a-a262-7fe93f0c5c56
md"g: $(@bind p1 PlutoUI.Slider(0.0:0.01:20, 10., true)) || r1: $(@bind p2 PlutoUI.Slider(0.0:0.0001:10., 0.2, true)) || T*: $(@bind p3 PlutoUI.Slider(0:0.1:1e3, 1e2, true))" 

# ╔═╡ 98c59f6d-8a43-49d3-ab3c-1d299a746656
md"T*reg: $(@bind p4 PlutoUI.Slider(0.0:10.0:1e5, 1e4, true)) || α: $(@bind p5 PlutoUI.Slider(0:10:1e5, 1e4, true)) || β: $(@bind p6 PlutoUI.Slider(0:0.5:40, 10.0, true))" 

# ╔═╡ 8568f550-6b8d-419d-a049-15e6513f919f
md"γ: $(@bind p7 PlutoUI.Slider(0:0.01:10, 5.0, true)) || δ1: $(@bind p8 PlutoUI.Slider(0:1e-5:1e-2, 1e-3, true)) || δ2: $(@bind p9 PlutoUI.Slider(0:1e-5:1e-2, 1e-3, true))" 

# ╔═╡ 7d8926a9-0978-4d12-a4dc-6523a0a502a5
md"κ: $(@bind p10 PlutoUI.Slider(0.0:1e-5:1e-3, 1e-4, true)) || μ: $(@bind p11 PlutoUI.Slider(0.0:1e4:1e6, 1e5, true)) || r2: $(@bind p12 PlutoUI.Slider(0.0:0.0001:10., 0.1, true))"

# ╔═╡ 0dea2ed4-dbb0-4b77-8c4d-d343bb3aa138
baseP = (g = p1, r1 = p2, T_star = p3, T_star_reg = p4, alpha = p5, beta = p6, gamma = p7, delta1 = p8, delta2 = p9, kappa = p10, mu = p11, r2 = p12)

# ╔═╡ 95e31467-914f-4448-b902-17ec2b25069e
let 

	fig = Figure(size = (800, 500))
	names = ["T-cells", "Infected\ncells", "Treg", "IL2"]

	# Jump version
	

	# Fader version
	sol2 = integrateAntagonismFader(baseP, u0base, times);
	fig2 = fig[1, 1]
	axes2 = []
	for (i, name) in enumerate(names)
		push!(axes2, Axis(fig2[i, 1], ylabel = name, yscale = log10))
		lines!(axes2[end], times, RELU.(sol2[i, :]))
	end
	lines!(axes2[2], times, RELU.(sol2[5, :]))
	
	fig
		
end

# ╔═╡ a4b126d3-a410-46b6-83e2-77a14dcf4252
md" ## Acute/chronic boundaries
Here I make some plots for the acute/chronic boundaries but for the full system
"

# ╔═╡ e45f37b1-8d03-4839-8af2-f1b0f6a65a53
function acuteFader(p, u0)
	# function to check if the system is in an acute state or a chronic state
	times = LinRange(0., 500, 1000)
	sol = integrateAntagonismFader(p, u0, times)

	# check if chronic
	chronic1 = (sol[2, end] < 0.2)
	chronic2 = (sol[5, end] < 0.2)
	if chronic1 && chronic2
		return 4
	elseif chronic1
		return 3
	elseif chronic2
...
# ╔═╡ ae5c4e4d-b76d-45c5-8106-824059b7d5d2
function makePlotFader(p, IC, fig; r1 = 0.2)
	# function to plot the result of infections
	# we start with a parameter that makes the first infection chronic, and look at how the second species changes the outcome

	# unpack parameters
	g, _, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = p
	u01, u02, u03, u04, u05 = IC


	# parameters to iterate over
	r2s = 10 .^ LinRange(-1, 1, 100)
	u05s = 10 .^ LinRange(-1, 3.5, 100)

	# matrix containing outcomes
	mat = fill(NaN, length(r2s), length(u05s))
	
	for (i, r2) in enumerate(r2s)
		for (j, u05) in enumerate(u05s)
			mat[i, j] = acuteFader(
	...
	ax1, hm = heatmap(
		fig[1, 1], 
		r2s, u05s, mat,
		colormap = cgrad(:lapaz, 4; categorical = true),
		colorrange = (0.5, 4.5),
		nan_color = :white,
		axis=(;yscale=log10, 
				xscale = log10, 
				ylabel=string(L"I_2(0)"), 
				xlabel = L"r_2",
				xgridvisible = false,
				ygridvisible = false,
				...
				title = string(#"Soft threshhold"
					"g=", g, "\tT*=", T_star, "\tTreg*=",T_star_reg, "\tα=", alpha, "\nβ=", beta, "\tγ=", gamma, "\tδ₁=", delta1, "\tδ₂=", delta2, "\nκ=", kappa, "\tμ=", mu, "\tr₁=", r1, "\tI₁(0)=", round(u02, digits=2)
				)
			)
	)
...
# ╔═╡ ecccb2be-cdda-43d6-b2a4-2a94780f9d39
# ╠═╡ disabled = true
#=╠═╡
let
	fig = Figure(size = (800, 300))

	makePlotFader(baseP, u0base, fig[1, 1], r1=6)
	makePlotFader(baseP, u0base, fig[1, 2])
...
# ╔═╡ e00cf718-0cc6-4f5c-b128-30dcedc64fc9
md" ## Acute chronic treatment
In the previous plot, we showed the effect of a second species added right at the start of the simulation. But in the optics of a treatment for a chronic disease, we look at the effect that adding a species once the infection has reached a chronic state.
"

# ╔═╡ ccb7a4a8-1ba8-4b36-a7ab-ab548f7a2625
function integrateAntagonismFaderTreatment(p, u0, tt, infection_time, pathogenAmount)

	#println("\nIC: ", u0)
	#println("Parameters: ", p)
	#println("infection time: ", infection_time)
	#println("pathogen amount: ", pathogenAmount)
	
	# defining callback
	condition(u, t, integrator) = (t == infection_time)
	affect!(integrator) = (integrator.u[5] += pathogenAmount)
	cb = DiscreteCallback(condition, affect!, save_positions = (false, false));

	# ODE problem definition
	prob = ODEProblem(fQuasistatic, [u0[1], u0[2], u0[3], u0[4], 0.0] , (tt[1], tt[end]), p, callback = cb);

	# Solving the system
	sol = solve(prob, Rodas4P(), saveat = tt[2]-tt[1], tstops=[infection_time], abstol=1e-9, reltol=1e-6);
	return sol
end

# ╔═╡ b4256582-0dbf-4ae0-a874-003f554540d5
function showRuns(p, u0, fig, treatmentTime, pathogenAmount; nameLabel = "")
	...
	# compute solution
	sol = integrateAntagonismFaderTreatment(p, u0, times, treatmentTime, pathogenAmount)

	T_traj = sol[1, :]
	T_traj[T_traj .< 1e-3] .= NaN
	I_traj = sol[2, :]
	I_traj[I_traj .< 1e-3] .= NaN

	I_traj_2 = sol[5, :]
	I_traj_2[I_traj_2 .< 1e-3] .= NaN
	...
	ax1 = Axis(
		...
		xlabel = "Time",
		yscale = log10,
		#title = string("r₂ = ", p[end], ", I₂(t₀) = ", pathogenAmount)
	)
	
	ax2 = Axis(fig[1, 1], yticklabelcolor = T_color, ylabel = "T", yscale = log10, ygridvisible = false)
	xlims!(ax1, 0, times[end])
	xlims!(ax2, 0, times[end])
	lines!(ax1, times, I_traj, color = I_color, linewidth = 4)
	lines!(ax1, times, I_traj_2, color = I_color_2, linewidth = 4, alpha = 0.7)
	lines!(ax2, times, T_traj, color = T_color, linewidth = 4, alpha = 0.7)
...
# ╔═╡ 78703c5a-3255-4243-a335-4f535f65b999
getRange(x) = maximum(x) - minimum(x) # helper function for the range of values for a series

# ╔═╡ b65a3ff4-745b-4fdd-985e-8d78db3c933f
function acuteFaderTreatment(p, u0, pathogenAmount; treatmentTime = 200)
	# function to check if the system is in an acute state or a chronic state
	times = LinRange(0., 800, 1000)
	threshOscillations = 1

	
	sol = integrateAntagonismFaderTreatment(p, u0, times, treatmentTime, pathogenAmount)
	#println("size of sol is: ", sol[2, end])

	# check if chronic or cyclic
	acute1 = (sol[2, end] < 0.2)
	acute2 = (sol[5, end] < 0.2)
	
	if acute1 && acute2 # both infections are acute
		return 9
		
	elseif acute1 # I1 acute, I2 chronic
		if getRange(sol[5, end-100:end]) > threshOscillations
			return 5 # I1 acute, I2 chronic and cyclic
		else
			return 7 # I1 acute, I2 chronic
		end
	elseif acute2 # I1 chronic, I2 acute
		if getRange(sol[2, end-100:end]) > threshOscillations
			return 4 # I1 chronic and cyclic, I2 acute
		else
			return 8 # I1 chronic, I2 acute
		end
	else
		cyclic1 = getRange(sol[2, end-100:end]) > threshOscillations
		cyclic2 = getRange(sol[5, end-100:end]) > threshOscillations
		if cyclic1 && cyclic2
			return 3
		elseif cyclic2
			return 2
		elseif cyclic1
			...
			return 6 # both chronic (not cyclic)
		end
	end
end

# ╔═╡ 16c912e8-ea8b-4478-8697-9227f8f114e6
function makePlotFaderTreatment(p, IC, fig; r1 = 0.2, colorbar = true, scatter_points_x = nothing, scatter_points_y = nothing, points_name = nothing)
	# function to plot the result of infections
	# we start with a parameter that makes the first infection chronic, and look at how the second species changes the outcome

	# unpack parameters
	g, _, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, _ = p
	u01, u02, u03, u04, u05 = IC
	...
	# parameters to iterate over
	r2s = 10 .^ LinRange(-1, 2, 101)
	u05s = 10 .^ LinRange(-1, 3.5, 101)
	...
	ax1, hm = heatmap(
		fig[1, 1], 
		r2s, u05s, mat,
		#colormap = cgrad(:Paired_10, 9; categorical = true),
		colormap = cgrad([cgrad(:Pastel1_5, 5; categorical = true)..., cgrad(:Paired_10, 4; categorical = true)...]; categorical=true),
		#colormap = [:red, :blue, :red, :blue, :red, :blue, :red, :blue, :red],
		colorrange = (0.5, 9.5),
		nan_color = :white,
		axis=(;yscale=log10, 
				xscale = log10,
				ylabel=string(L"I_2(t_\text{treatment})"), 
				xlabel = L"r_2",
				xgridvisible = false,
				ygridvisible = false,
				xminorticksvisible=true,
	...
	# scatter points
	if (scatter_points_x, scatter_points_y, points_name) != (nothing, nothing, nothing)
		scatter!(ax1, scatter_points_x, scatter_points_y, color = :white, strokecolor=:black, strokewidth = 1)

		label_size = 25
		for (i,j,k) in zip(scatter_points_x, scatter_points_y, points_name)
			text!(ax1, i,j, text = k, fontsize = label_size, color = :white, strokecolor = :black, strokewidth = 1)
		end
	end
	...
	if colorbar == true
		...
		cb = Colorbar(
			...
			ticks = (1:9, reverse([ 
				...
				"I₁: Acute, I₂: Chronic",
				"I₁: Chronic, I₂: Chronic",
				"I₁: Acute, I₂: Chronic (cyclic)",
				"I₁: Chronic (cyclic), I₂: acute",
				"I₁: Chronic (cyclic), I₂: Chronic (cyclic)",
				"I₁: Chronic, I₂: Chronic (cyclic)",
				"I₁: Chronic (cyclic), I₂: Chronic"
			]))
		)
		cb.alignmode = Mixed(right = 0)
	end
	ax1.alignmode = Mixed(left = 0)
	#ax1.width = 350
	#ax1.tellwidth = false
...
# ╔═╡ 7194990a-1123-454a-987b-b82565b448f2


# ╔═╡ 5a7bc7ad-3862-4dcd-ac06-8932253cab72
function plotRuns1(fig)

	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = baseP

	r1 = 0.5


	showRuns(
		[g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 40.], 
		[1e-3, 1.0, 1e4, 0.0], 
		fig[1, 1],
		25,
		10,
	...
	showRuns(
		[g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 1.], 
		[1e-3, 1.0, 1e4, 0.0], 
		fig[1, 2],
		25,
		0.1,
	...
	Box(fig[1, 3], color = :grey70)
	
	showRuns(
		[g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 8], 
		[1e-3, 1.0, 1e4, 0.0], 
		fig[2, 1],
		25,
		10,
	...
	showRuns(
		[g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 20], 
		[1e-3, 1.0, 1e4, 0.0], 
		fig[2, 3],
		25,
		10,
	...
	Box(fig[2, 3], color = :grey70)

	showRuns(
		[g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 0.2], 
		[1e-3, 1.0, 1e4, 0.0], 
		fig[3, 1],
		25,
		400,
	...
	Box(fig[3, 3], color = :grey70)


	label_size = 16
	Label(fig[0, 1], "I₁\nAcute", fontsize = label_size, font = :bold, tellwidth = false, tellheight = false)
	Label(fig[0, 2], "I₁\nChronic", fontsize = label_size, font = :bold, tellwidth = false, tellheight = false)
	Label(fig[0, 3], "I₁\nChronic\nCyclic", fontsize = label_size, font = :bold, tellwidth = false, tellheight = false)
	Label(fig[1, 0], "I₂\nAcute", fontsize = label_size, font = :bold, tellwidth = false, tellheight = false)
	Label(fig[2, 0], "I₂\nChronic", fontsize = label_size, font = :bold, tellwidth = false, tellheight = false)
	Label(fig[3, 0], "I₂\nChronic\nCyclic", fontsize = label_size, font = :bold, tellwidth = false, tellheight = false)
...
# ╔═╡ 2be784c5-52af-4c6d-af11-cd1cea3a7406
function plotRuns2(fig)
...
# ╔═╡ 8c92a1ea-3d13-43ab-93d7-2a1891b63d47
let
	fig = Figure(size = (1800, 700))

	makePlotFaderTreatment(
		baseP, u0base, fig[1, 1], r1 = 0.5, colorbar = true, 
		scatter_points_x = [40, 8, 1.0, 1.0, 0.2], 
		scatter_points_y = [10, 10, 10, 0.1, 400], 
		points_name = ["1", "4", "5", "2", "7"]
	)


	# making the subplot for the first phase diagram
	oscSubFig1 = fig[2, 1] = GridLayout()
	plotRuns1(oscSubFig1)


	makePlotFaderTreatment(baseP, u0base, fig[1, 2], r1 = 0.2, colorbar = true,
		scatter_points_x = [40, 8, 1.0, 1.0, 0.2], 
		scatter_points_y = [10, 10, 10, 0.1, 400], 
		points_name = ["1", "4", "5", "2", "7"]
	)
	oscSubFig2 = fig[2, 2] = GridLayout()
	plotRuns2(oscSubFig2)
	...
	labelsize = 30
	pad = (0, 5, 6, 0)
	Label(fig[1, 1, TopLeft()], "A)", fontsize = labelsize, font = :bold, padding = pad, halign = :right)
	Label(fig[2, 1, TopLeft()], "B)", fontsize = labelsize, font = :bold, padding = pad, halign = :right)
	Label(fig[1, 2, TopLeft()], "C)", fontsize = labelsize, font = :bold, padding = pad, halign = :right)
	Label(fig[2, 2, TopLeft()], "D)", fontsize = labelsize, font = :bold, padding = pad, halign = :right)


	save("makieExports/figS7.png", fig)

	
	fig
end

# ╔═╡ 952b1cca-8024-4076-8727-1bb7531bccd0
function integrateAntagonismFaderInitial(p, u0, tt)

	# ODE problem definition
	prob = ODEProblem(fQuasistatic, [u0[1],u0[2], u0[3], u0[4], u0[5]] , (tt[1], tt[end]), p);

	# Solving the system
	sol = solve(prob, Rodas4P(), abstol=1e-9, reltol=1e-6, saveat = tt[2]-tt[1]);
	return sol
end

# ╔═╡ 86b247bb-1ec6-46dd-91ad-f6e5368e06a2
function acuteFaderInitial(p, u0)
	# function to check if the system is in an acute state or a chronic state
	times = LinRange(0., 800, 1000)
	threshOscillations = 1
	
	sol = integrateAntagonismFaderInitial(p, u0, times)
...
# ╔═╡ 061f7465-3825-4ae0-80bb-69f1049c306c
function makePlotFaderTreatment2(p, IC, fig; r1 = 0.2, colorbar = true, scatter_points_x = nothing, scatter_points_y = nothing, points_name = nothing)
	# function to plot the result of infections
	# we start with a parameter that makes the first infection chronic, and look at how the second species changes the outcome
...
# ╔═╡ 2c575e40-7520-40d5-814b-f0f48f994780
function showRunsInitial(p, u0, fig; nameLabel = "")
...
# ╔═╡ f6b7df0f-7dd9-49eb-aa2d-ee2a90a1772f
function plotRuns3(fig)
	...
	showRunsInitial(
		[g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 10.], 
		[1e-10, 10.0, 1e4, 0, 10], 
		fig[1, 1],
		nameLabel="1"
	)
	...
	showRunsInitial(
		[g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 2], 
		[1e-10, 10.0, 1e4, 0.0, 10.0], 
		fig[2, 1],
		nameLabel="4"
	)
	showRunsInitial(
		[g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 1], 
		[1e-10, 10.0, 1e4, 0.0, 10], 
		fig[2, 2],
		nameLabel="5"
	)
...
# ╔═╡ da66c244-c206-4d5b-a614-15f40bd33ecf
function plotRuns4(fig)
...
# ╔═╡ 3605dead-8c1e-4064-9181-749b77232064
let
	fig = Figure(size = (1800, 700))

	u1, u2, u3, u4, u5 = u0base
	
	makePlotFaderTreatment2(
		baseP, [u1, 10, u3, u4, u5], fig[1, 1], r1 = 0.5, colorbar = true, 
		scatter_points_x = [10, 2, 1.0, 1.0, 0.2], 
		scatter_points_y = [10, 10, 10, 0.1, 400], 
		points_name = ["1", "4", "5", "2", "7"]
	)
	...
	# making the subplot for the first phase diagram
	oscSubFig1 = fig[2, 1] = GridLayout()
	plotRuns3(oscSubFig1)


	makePlotFaderTreatment2(
		baseP, [u1, u2, u3, u4, u5], fig[1, 2], r1 = 0.2, colorbar = true, 
		scatter_points_x = [10, 2, 1.0, 1.0, 0.2], 
		scatter_points_y = [10, 10, 10, 0.1, 400], 
		points_name = ["1", "4", "5", "2", "7"]
	)
	oscSubFig2 = fig[2, 2] = GridLayout()
	plotRuns4(oscSubFig2)
	...
	save("makieExports/figS9.png", fig)
...
manifest_format = "2.0"
project_hash = "43713850e575150ee41c0c9052cf96a9a314ba6f"
