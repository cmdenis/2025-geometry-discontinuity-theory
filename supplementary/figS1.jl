...
# ╔═╡ 80120104-4661-4c50-bcfc-872bb0aa1fb9
function diffEquationJump(du, u, p, t)
	g, r, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu = p

	# T-cells
	du[1] = - gamma*u[1] + kappa*u[2]*(
		T_star + alpha/delta1*(u[1]*u[4]/u[3])
	)

	# infected cells
	if u[2] >= 1
		du[2] = (g + r*u[2] - kappa*u[2]*(u[1] + T_star))
	else
		du[2] = -u[2]
	end
	...
	# IL2
	du[4] = - u[4]*(alpha*u[1]  + beta*u[3]) + mu*u[1]
end

# ╔═╡ f5262008-148c-4dcf-90da-4fc49900e2db
function integrateJump(p, u0, times)
	prob = ODEProblem(diffEquationJump, u0, (times[1], times[end]), p);
...
function showRuns(p, u0, fig)
	...
	lines!(ax1, times, T_traj, color = T_color, linewidth = 4)
	lines!(ax2, times, I_traj, color = I_color, linewidth = 4)
	...
	ax3 = Axis(
		fig[2, 1], 
		yticklabelcolor = Treg_color, 
		ylabel = "Treg", 
		xlabel = "Time",
		yscale = log10,
	)
	ax4 = Axis(fig[2, 1], yticklabelcolor = IL2_color, ylabel = "IL2", ygridvisible = false)
	lines!(ax3, times, sol[3, :], color = Treg_color, linewidth = 4, alpha = 0.7)
	lines!(ax4, times, sol[4, :], color = IL2_color, linewidth = 4, alpha = 0.7)
	linkxaxes!(ax3, ax4)
...
let
	fig = Figure(size = (900, 600), figure_padding = 0)
	...
	width = 320
	colsize!(thirdFig, 1, width)
	colsize!(fig.layout, 1, width)
	colsize!(fig.layout, 2, width)
	...
	Label(thirdFig[1, 1, TopLeft()], "C)", fontsize = label_size, font = :bold, padding = pad, halign = :right)

	save("makieExports/figS1.png", fig)
	fig
end
...
manifest_format = "2.0"
project_hash = "bdb18a06eabb9e6f8d7c0948d989a2de78ceee53"
