...
# ╔═╡ a5d70c5d-2c99-4ef5-830d-2601c4496a0c
function diffEquationAntagonismFader(du, u, p, t)
	g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, r2 = p

	# T-cells
	du[1] = kappa*(u[2] + u[5])*(T_star + alpha/delta1*(u[1]*u[4]/u[3])) - gamma*u[1]

	# infected cells
	n = 4
	k = 1
	du[2] = (g + r1*u[2] - kappa*u[2]*(u[1] + T_star))*crossfader(u[2], n, k) - u[2]*(1-crossfader(u[2], n, k))

	# Tregs
	du[3] = beta*u[3]*u[4] - delta2*u[3]*(u[3] - T_star_reg)

	# IL2
	du[4] = mu*u[1]  -  u[4]*(alpha*u[1]  + beta*u[3])

	# infected cells 2
	du[5] = (g + r2*u[5] - kappa*u[5]*(u[1] + T_star))*crossfader(u[5], n, k) - u[5]*(1-crossfader(u[5], n, k))
end

# ╔═╡ 1790350b-eefc-4602-98a1-091d06d5e818
function integrateAntagonismFader(p, u0, times)
	prob = ODEProblem(diffEquationAntagonismFader, u0, (times[1], times[end]), p);
	sol = solve(prob, Rodas4P(), saveat = times[2]-times[1]);
	return sol
end
...
md"log₁₀(T(0)): $(@bind u01 PlutoUI.Slider(-4:0.001:4, -3., true)) || log₁₀(I₁(0)): $(@bind u02 PlutoUI.Slider(-1.0:0.01:4.0, 0.0, true))"

# ╔═╡ af20fc9f-1818-4b34-8d7e-ecbed9756cc8
md"log₁₀(I₂(0)): $(@bind u05 PlutoUI.Slider(-1.0:0.01:4.0, 0.0, true))"
...
function integrateAntagonismFaderTreatment(p, u0, tt, infection_time, pathogenAmount)
	...
	prob = ODEProblem(diffEquationAntagonismFader, [u0[1],u0[2], u0[3], u0[4], 0.0] , (tt[1], tt[end]), p, callback = cb);
...
# ╔═╡ bf45cfb6-97dc-4c59-a9b5-785c4fc91164
function integrateAntagonismFaderInitial(p, u0, tt)

	# ODE problem definition
	prob = ODEProblem(diffEquationAntagonismFader, [u0[1],u0[2], u0[3], u0[4], u0[5]] , (tt[1], tt[end]), p);
...
# ╔═╡ 98592406-7272-471b-861a-4db620d812e4
...
# ╔═╡ 19bc5f6f-2f2d-422a-a9ec-4b5afaddd2b5
...
# ╔═╡ f74651fa-ec87-432c-84f3-992079aa2544
...
function plotRuns2(fig)
	...
	showRuns(
		[g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 70.], 
		[1e-3, 1.0, 1e4, 0.0], 
		fig[1, 1],
		25,
		10,
...
# ╔═╡ f0e09297-a67d-40bd-aef9-38f7b38646bf
let
	fig = Figure(size = (1800, 700))

	makePlotFaderTreatment(
		baseP, u0base, fig[1, 1], r1 = 0.5, colorbar = true, 
		scatter_points_x = [60, 20, 1.0, 1.0, 0.2], 
		scatter_points_y = [10, 10, 10, 0.1, 700], 
		points_name = ["1", "4", "5", "2", "7"]
	)
	#makePlotFaderTreatment(baseP, u0base, fig[2, 1], colorbar=false, scatter_points_x = [1.0, 1.0, 1.0, 20], scatter_points_y = [1000, 10, 0.1, 10], points_name = ["b", "a", "c", "d"])
	...
	makePlotFaderTreatment(baseP, u0base, fig[1, 2], r1 = 0.2, colorbar = true,
		scatter_points_x = [50, 20, 1.0, 1.0, 0.2], 
		scatter_points_y = [10, 10, 10, 0.1, 750], 
		points_name = ["1", "4", "5", "3", "7"]
	)
	...
	save("makieExports/figS6.png", fig)
...
function plotRuns3(fig)
	...
	showRunsInitial(
		[g, r1, T_star, T_star_reg, alpha, beta, gamma, delta1, delta2, kappa, mu, 4], 
...
let
	...
	makePlotFaderTreatment2(
		...
		scatter_points_x = [10, 4, 1.0, 1.0, 0.2], 
		scatter_points_y = [10, 10, 10, 0.1, 700], 
		points_name = ["1", "4", "5", "2", "7"]
	)
	#makePlotFaderTreatment(baseP, u0base, fig[2, 1], colorbar=false, scatter_points_x = [1.0, 1.0, 1.0, 20], scatter_points_y = [1000, 10, 0.1, 10], points_name = ["b", "a", "c", "d"])
	...
	save("makieExports/figS8.png", fig)
